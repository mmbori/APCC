...
LT_SCOPE void *	slist_remove	(SList **phead, void * (*find)(SList *item, void *userdata),
				 void *matchdata);
LT_SCOPE SList *slist_reverse	(SList *slist);
LT_SCOPE SList *slist_sort	(SList *slist, int (*compare)(const SList *item1, const SList *item2, void *userdata),
				 void *userdata);

LT_SCOPE SList *slist_tail	(SList *slist);
LT_SCOPE SList *slist_nth	(SList *slist, size_t n);
LT_SCOPE void *	slist_find	(SList *slist, void * (*find)(SList *item, void *userdata),
				 void *matchdata);
LT_SCOPE size_t slist_length	(SList *slist);

LT_SCOPE void *	slist_foreach   (SList *slist, void * (*foreach)(SList *item, void *userdata),
				 void *userdata);
