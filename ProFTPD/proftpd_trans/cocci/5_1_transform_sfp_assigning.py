...
def generate_memcpy_transformation_rules(fp_data: Dict[str, List[str]], 
    ...
    for fp_name in sorted(fp_data.keys()):
        ...
        for func_name in valid_functions:
            ...
            if func == '0':
                out.append(f"""
// Rule: .{fp_name} = 0 ==> .{fp_name}_signature = {fp_name}_signatures[{fp_name}_0];
@{rule_name}@
expression E;
...
(
E.FP_NAME = 0;
+ E.{fp_name}_signature = {fp_name}_signatures[{fp_name}_0];
|
E->FP_NAME = 0;
+ E->{fp_name}_signature = {fp_name}_signatures[{fp_name}_0];
)
""")
                ...
                out.append(f"""
// Rule: .{fp_name} = 0 ==> .{fp_name}_signature = {fp_name}_signatures[{fp_name}_NULL];
@{rule_name}@
expression E;
identifier FP_NAME = {fp_name};
@@
(
E.FP_NAME = 0;
+ E.{fp_name}_signature = {fp_name}_signatures[{fp_name}_NULL];
|
E->FP_NAME = 0;
+ E->{fp_name}_signature = {fp_name}_signatures[{fp_name}_NULL];
)
""")
                ...
                out.append(f"""
// Rule: .{fp_name} = {func} ==> .{fp_name}_signature = {fp_name}_signatures[{fp_name}_{func}];
@{rule_name}@
expression E;
identifier FP_NAME = {fp_name};
...
(
E.FP_NAME = FUNC_NAME;
+ E.{fp_name}_signature = {fp_name}_signatures[{fp_name}_{func}];
|
E.FP_NAME = &FUNC_NAME;
+ E.{fp_name}_signature = {fp_name}_signatures[{fp_name}_{func}];
|
...
position p;
@@
E.{fp_name}_signature@p = {fp_name}_signatures[{fp_name}_{func}];
