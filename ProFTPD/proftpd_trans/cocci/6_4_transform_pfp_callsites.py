...
Usage:
  python3 6_4_transform_pfp_callsites.py --json data.json --output transform.cocci
"""
...
def generate_transformation_rules(fp_data: Dict, removed_functions: Set[str]) -> str:
    ...
    out.append("// Uses int comparison with signatures enum array\n\n")
    ...
    for (fn_name, fp_name), info in sorted(fp_data.items()):
        ...
        if len(valid_funcs) == 1:
            ...
            # Multiple functions: if-else chain with int comparison
            ...
            for func in valid_funcs:
                if first:
                    out.append(f"+     if ({fp_name}_signature == {fp_name}_signatures[{fp_name}_{func}]) {{\n")
                    first = False
                else:
                    out.append(f"+     }}\n")
                    out.append(f"+     else if ({fp_name}_signature == {fp_name}_signatures[{fp_name}_{func}]) {{\n")
