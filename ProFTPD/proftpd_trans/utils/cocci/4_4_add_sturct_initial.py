    ...
    for match in init_pattern.finditer(content):
            ...
            if idx in fp_index_map:
                ...
                if func_match:
                    ...
                    if func_name in ('0', 'NULL'):
                        fp_assignments[fp_name] = '0'
                    elif func_name.isidentifier():
                        # JSON에서 이 fp_name에 할당 가능한 함수인지 확인
                        if fp_name in fp_to_functions:
                            if func_name in fp_to_functions[fp_name]:
                                fp_assignments[fp_name] = func_name
                            elif verbose:
                                print(f"    [WARN] {fp_name}: {func_name}이(가) JSON에 없음")
        ...
        for fp_name, _ in sorted(fp_list, key=lambda x: x[1]):
            if fp_name in fp_assignments:
                ...
                if func_name == '0':
                    signature_lines.append(f"  .{fp_name}_signature = {fp_name}_signatures[{fp_name}_0]")
                elif func_name == 'NULL' :
                    signature_lines.append(f"  .{fp_name}_signature = {fp_name}_signatures[{fp_name}_NULL]")
                else:
                    signature_lines.append(f"  .{fp_name}_signature = {fp_name}_signatures[{fp_name}_{func_name}]")
...
def main():
    ...
    # 파일 검색
    file_patterns = [
        os.path.join(args.source_dir, "*.c"),
        os.path.join(args.source_dir, "*.h"),
        os.path.join(args.source_dir, "*.in"),
        os.path.join(args.source_dir, "**/*.c"),
        os.path.join(args.source_dir, "**/*.h"),
        os.path.join(args.source_dir, "**/*.in")
    ]
