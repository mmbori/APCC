...
class FPSignatureGenerator:
    def __init__(self, json_file: str, output_header: str, output_source: str, 
    ...
    def generate_header_content(self) -> str:
        ...
        lines.append("#include \"jot.h\"")
        lines.append("typedef struct wrap2_conn_t wrap2_conn_t;")
        lines.append("typedef struct quota_table_t quota_table_t;")
        lines.append("typedef struct tls_ocsp_cache_t tls_ocsp_cache_t;")
        lines.append("typedef struct OCSP_RESPONSE OCSP_RESPONSE;")
        lines.append("typedef struct tls_sess_cache_t tls_sess_cache_t;")
        lines.append("typedef struct SSL_SESSION SSL_SESSION;")
        lines.append("typedef struct wrap2_table_t wrap2_table_t;")
        lines.append("typedef struct quota_type_t quota_type_t;")
        lines.append("typedef struct sftp_kbdint_driver_t sftp_kbdint_driver_t;")
        ...
        for fp_name in sorted(self.fp_functions_map.keys()):
            ...
            for i, func in enumerate(functions):
                ...
                lines.append(f"    {fp_name}_{func} = {i}{comma}")
            lines.append(f"}} {fp_name}_enum;")
            lines.append("")
            
            # 2. Signature 배열 선언
            lines.append(f"// {fp_name} signatures (int)")
            lines.append(f"extern int {fp_name}_signatures[{len(functions)}];")
            lines.append("")
            
            # 3. 초기화 함수 선언
            lines.append(f"// {fp_name} signature initialization")
            lines.append(f"void init_{fp_name}_signatures(void);")
            lines.append("")
        
        # 전역 초기화 함수
        lines.append("// Global initialization")
    ...
    def generate_source_content(self) -> str:
        ...
        # Header 포함
        lines.append("/*")
        lines.append(" * Function Pointer Signatures - Implementation")
        lines.append(" * Auto-generated - DO NOT EDIT")
        lines.append(" */")
        lines.append("")
        lines.append(f'#include "{os.path.basename(self.output_header)}"')
        lines.append("")
        ...
        for fp_name in sorted(self.fp_functions_map.keys()):
            ...
            # 1. Signature 배열 정의
            lines.append(f"// {fp_name} signatures")
            lines.append(f"int {fp_name}_signatures[{len(functions)}] = {{0}};")
            lines.append("")
            
            # 2. 초기화 함수 정의
            lines.append(f"// Initialize {fp_name} signatures")
            lines.append(f"void init_{fp_name}_signatures(void) {{")
            lines.append("    static int initialized = 0;")
            lines.append("    if (initialized) return;")
            lines.append("    initialized = 1;")
            lines.append("")
            lines.append(f"    // Fill signatures with random data")
            lines.append(f"    for (int i = 0; i < {len(functions)}; i++) {{")
            lines.append(f"        {fp_name}_signatures[i] = (int)rand();")
            lines.append("    }")
            lines.append("")
            
            # 함수 매핑 주석
            for i, func in enumerate(functions):
                lines.append(f"    // [{i}] {fp_name}_{func} = {func}")
    ...
    def generate_files(self):
        ...
        try:
            ...
            # 소스 파일 생성
            source_content = self.generate_source_content()
            with open(self.output_source, 'w', encoding='utf-8') as f:
                f.write(source_content)
            print(f"✓ 소스 파일 생성: {self.output_source}")
...
생성되는 코드:
  ...
  - init_{fp}_signatures(): 초기화 함수
    ...
    parser.add_argument('output_source', help='출력 소스 파일')
