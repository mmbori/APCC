...
static void send_ascii_command(const char *buf) {
    ...
    do {
        ssize_t nw = tcp_write((void*)con, ptr + offset, len - offset);
        if (nw == -1) {
            if (errno != EINTR) {
...
static void read_ascii_response(char *buffer, size_t size) {
    ...
    bool need_more = true;
    do {
        // fprintf(stderr, "read_ascii_response before tcp_read\n");
        ssize_t nr = tcp_read(con, buffer + offset, 1);
        // ssize_t nr = con->read(con, buffer + offset, 1);
        // fprintf(stderr, "read_ascii_response after tcp_read\n");
...
static enum test_return test_issue_92(void) {
    ...
    fprintf(stderr, "test_issue_92 done\n");
...
static enum test_return test_issue_102(void) {
    ...
    fprintf(stderr, "test_issue_102 connect server\n");

    send_ascii_command(buffer);

    fprintf(stderr, "test_issue_102 send_ascii_command\n");
    /* verify that the server closed the connection */
    assert(tcp_read(con, buffer, sizeof(buffer))== 0);

    fprintf(stderr, "test_issue_102 tcp_read\n");
    ...
    fprintf(stderr, "test_issue_102 while loop over\n");

    send_ascii_command(buffer);
    usleep(5);

    fprintf(stderr, "test_issue_102 usleep 5\n");

    send_ascii_command("\r\n");
    fprintf(stderr, "test_issue_102 send_ascii_command\n");
    char rsp[80];
    read_ascii_response(rsp, sizeof(rsp));
    fprintf(stderr, "test_issue_102 read_ascii_response\n");
    ...
    fprintf(stderr, "test_issue_102 assert strncmp END\n");
    ...
    fprintf(stderr, "test_issue_102 done\n");
...
static void safe_send(const void* buf, size_t len, bool hickup)
{
    ...
    do {
        ...
        ssize_t nw = tcp_write(con, ptr + offset, num_bytes);
        if (nw == -1) {
            if (errno != EINTR) {
                fprintf(stderr, "Failed to write: %s\n", strerror(errno));
                abort();
...
static bool safe_recv(void *buf, size_t len) {
    ...
    off_t offset = 0;
    do {
        ssize_t nr = tcp_read(con, ((char*)buf) + offset, len - offset);
        if (nr == -1) {
            if (errno != EINTR) {
                fprintf(stderr, "Failed to read: %s\n", strerror(errno));
                abort();
...
static enum test_return test_binary_quit_impl(uint8_t cmd) {
    ...
    assert(tcp_read(con, buffer.bytes, sizeof(buffer.bytes))== 0);
    close_conn();
    con = connect_server("127.0.0.1", port, false, enable_ssl);
    assert(con);
...
static enum test_return test_binary_illegal(void) {
    ...
    while (cmd != 0x00) {
        ...
        validate_response_header(&buffer.response, cmd,
                                ...
                                //  fprintf(stderr, "test_binary_illegal validate_response_header\n");
        ++cmd;
    }
...
static enum test_return test_binary_pipeline_hickup(void)
{
    ...
    fprintf(stderr, "test_binary_pipeline_hickup pthread_create\n");
    ...
    fprintf(stderr, "test_binary_pipeline_hickup test_binary_pipeline_hickup_chunk\n");
    ...
    fprintf(stderr, "test_binary_pipeline_hickup safe_send\n");

    pthread_join(tid, NULL);
    fprintf(stderr, "test_binary_pipeline_hickup pthread_join\n");
    free(buffer);

    fprintf(stderr, "test_binary_pipeline_hickup free\n");
    return TEST_PASS;
}
...
struct testcase {
    ...
    enum test_return (*function)(void);
