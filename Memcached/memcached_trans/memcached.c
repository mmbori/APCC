...
ssize_t tcp_write(conn *arg, void *buf, size_t count);
...
void thread_io_queue_add(LIBEVENT_THREAD *t, int type, void *ctx, void (*cb)(io_queue_t *q)) {
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (ssl) {
        ...
        c->read = tcp_read;
        c->read_signature = read_tcp_read;
        c->sendmsg = tcp_sendmsg;
        c->sendmsg_signature = sendmsg_tcp_sendmsg;
        c->write = tcp_write;
        c->write_signature = write_tcp_write;
        c->ssl_enabled = false;
    }

    if (IS_UDP(transport)) {
        c->try_read_command = try_read_command_udp;
        c->try_read_command_signature = try_read_command_try_read_command_udp;
    } else {
        switch (c->protocol) {
            case ascii_prot:
                if (settings.auth_file == NULL) {
                    ...
                    c->try_read_command_signature = try_read_command_try_read_command_ascii;
                } else {
                    c->authenticated = false;
                    c->try_read_command = try_read_command_asciiauth;
                    c->try_read_command_signature = try_read_command_try_read_command_asciiauth;
                }
                break;
            case binary_prot:
                ...
                c->try_read_command_signature = try_read_command_try_read_command_binary;
                break;
            case negotiating_prot:
                c->try_read_command = try_read_command_negotiate;
                c->try_read_command_signature = try_read_command_try_read_command_negotiate;
                break;
#ifdef PROXY
            case proxy_prot:
...
void append_stat(const char *name, void (*add_stats)(const char *key, const uint16_t klen,
                         const char *val, const uint32_t vlen,
                         const void *cookie), conn *c,
                 const char *fmt, ...) {
...
void server_stats(void (*add_stats)(const char *key, const uint16_t klen,
                         const char *val, const uint32_t vlen,
                         const void *cookie), void *c) {
    pid_t pid = getpid();
...
void process_stat_settings(void (*add_stats)(const char *key, const uint16_t klen,
                         const char *val, const uint32_t vlen,
                         const void *cookie), void *c) {
    assert(add_stats);
...
bool get_stats(const char *stat_type, int nkey, void (*add_stats)(const char *key, const uint16_t klen,
                         const char *val, const uint32_t vlen,
                         const void *cookie), void *c) {
    bool ret = true;
...
void process_stats_conns(void (*add_stats)(const char *key, const uint16_t klen,
                         const char *val, const uint32_t vlen,
                         const void *cookie), void *c) {
    int i;
...
int try_read_command_negotiate(conn *c) {
    ...
    if (c->try_read_command_signature == try_read_command_try_read_command_ascii) {
        return try_read_command_ascii(c);
    } else if (c->try_read_command_signature == try_read_command_try_read_command_asciiauth) {
        return try_read_command_asciiauth(c);
    } else if (c->try_read_command_signature == try_read_command_try_read_command_binary) {
        return try_read_command_binary(c);
    } else if (c->try_read_command_signature == try_read_command_try_read_command_negotiate) {
        return try_read_command_negotiate(c);
    } else if (c->try_read_command_signature == try_read_command_try_read_command_udp) {
        return try_read_command_udp(c);
    }
}
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        ...
        res = tcp_read(c, c->rbuf + c->rbytes, avail);
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.bytes_read += res;
            pthread_mutex_unlock(&c->thread->stats.mutex);
...
static int read_into_chunked_item(conn *c) {
    ...
    while (c->rlbytes > 0) {
        ...
        if (c->rbytes > 0) {
            ...
            res = tcp_read(c, ch->data + ch->used,
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_parse_cmd:
            c->noreply = false;
            if (c->try_read_command_signature == try_read_command_try_read_command_ascii) {
    try_read_command_ascii(c);
} else if (c->try_read_command_signature == try_read_command_try_read_command_asciiauth) {
    try_read_command_asciiauth(c);
} else if (c->try_read_command_signature == try_read_command_try_read_command_binary) {
    try_read_command_binary(c);
} else if (c->try_read_command_signature == try_read_command_try_read_command_negotiate) {
    try_read_command_negotiate(c);
} else if (c->try_read_command_signature == try_read_command_try_read_command_udp) {
    try_read_command_udp(c);
}
if (c == 0) {
                /* we need more data! */
                if (c->resp_head) {
        ...
        case conn_nread:
            ...
            if (c->item_malloced || ((((item *)c->item)->it_flags & ITEM_CHUNKED) == 0) ) {
                ...
                /*  now try reading from the socket */
                res = tcp_read(c, c->ritem, c->rlbytes);
                if (res > 0) {
                    pthread_mutex_lock(&c->thread->stats.mutex);
                    c->thread->stats.bytes_read += res;
                    pthread_mutex_unlock(&c->thread->stats.mutex);
        ...
        case conn_swallow:
            ...
            /*  now try reading from the socket */
            res = tcp_read(c, c->rbuf, c->rsize > c->sbytes ? c->sbytes : c->rsize);
            if (res > 0) {
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.bytes_read += res;
                pthread_mutex_unlock(&c->thread->stats.mutex);
