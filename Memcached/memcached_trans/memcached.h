...
#define APPEND_NUM_FMT_STAT(name_fmt, num, name, fmt, val)          \
    ...
    append_stats(key_str, klen, val_str, vlen, c);
...
typedef struct io_queue_s {
    ...
    iop_head_t stack;
    void (*submit_cb)(io_queue_t *q); // callback given a full stack of pending IO's at once.
    int type;

    int submit_cb_signature;
} io_queue_t;
...
struct _io_pending_t {
    ...
    void (*return_cb)(io_pending_t *pending); // called on worker thread.
    void (*finalize_cb)(io_pending_t *pending); // called back on the worker thread.
    STAILQ_ENTRY(_io_pending_t) iop_next; // queue chain.
    char data[120];

    int return_cb_signature;
    int finalize_cb_signature;
};
...
struct conn {
    ...
    int try_read_command_signature;
    int read_signature;
    int sendmsg_signature;
    int write_signature;
};
