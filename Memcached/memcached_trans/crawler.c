...
typedef struct {
    int (*init)(crawler_module_t *cm, void *data); /* run before crawl starts */
    void (*eval)(crawler_module_t *cm, item *it, uint32_t hv, int slab_cls); /* runs on an item. */
    void (*doneclass)(crawler_module_t *cm, int slab_cls); /* runs once per sub-crawler completion. */
    void (*finalize)(crawler_module_t *cm); /* runs once when all sub-crawlers are done. */
...
crawler_module_reg_t crawler_expired_mod = {
    ...
    .init_signature = init_crawler_expired_init,
    .eval = crawler_expired_eval,
    .eval_signature = eval_crawler_expired_eval,
    .doneclass = crawler_expired_doneclass,
    .doneclass_signature = doneclass_crawler_expired_doneclass,
    .finalize = crawler_expired_finalize,
    .finalize_signature = finalize_crawler_expired_finalize,
    .needs_lock = true,
    .needs_client = false,
};

int crawler_metadump_init(crawler_module_t *cm, void *data);
void crawler_metadump_eval(crawler_module_t *cm, item *search, uint32_t hv, int i);
void crawler_metadump_finalize(crawler_module_t *cm);

crawler_module_reg_t crawler_metadump_mod = {
    .init = crawler_metadump_init,
    .init_signature = init_crawler_metadump_init,
    .eval = crawler_metadump_eval,
    .eval_signature = eval_crawler_metadump_eval,
    .doneclass = NULL,
    .finalize = crawler_metadump_finalize,
    .finalize_signature = finalize_crawler_metadump_finalize,
    .needs_lock = false,
    .needs_client = true,
};

int crawler_mgdump_init(crawler_module_t *cm, void *data);
void crawler_mgdump_eval(crawler_module_t *cm, item *search, uint32_t hv, int i);
void crawler_mgdump_finalize(crawler_module_t *cm);

crawler_module_reg_t crawler_mgdump_mod = {
    .init = crawler_mgdump_init,
    .init_signature = init_crawler_mgdump_init,
    .eval = crawler_mgdump_eval,
    .eval_signature = eval_crawler_mgdump_eval,
    .doneclass = NULL,
    .finalize = crawler_mgdump_finalize,
    .finalize_signature = finalize_crawler_mgdump_finalize,
    .needs_lock = false,
    .needs_client = true,
};
...
static void lru_crawler_class_done(int i) {
    ...
    if (active_crawler_mod.mod->doneclass != NULL)
        crawler_expired_doneclass(&active_crawler_mod, i);
}
...
static void item_crawl_hash(void) {
    ...
    while (assoc_iterate(iter, &it)) {
        ...
        // for expire eval.
        if (active_crawler_mod.mod->eval_signature == eval_crawler_expired_eval) {
            crawler_expired_eval(&active_crawler_mod, it, 0, 0);
        } else if (active_crawler_mod.mod->eval_signature == eval_crawler_metadump_eval) {
            crawler_metadump_eval(&active_crawler_mod, it, 0, 0);
        } else if (active_crawler_mod.mod->eval_signature == eval_crawler_mgdump_eval) {
            crawler_mgdump_eval(&active_crawler_mod, it, 0, 0);
        }
        crawls_persleep--;
        items++;
    }
...
static void *item_crawler_thread(void *arg) {
    ...
    while (crawler_count) {
        ...
        for (i = POWER_SMALLEST; i < LARGEST_ID; i++) {
            ...
            if (active_crawler_mod.mod->eval_signature == eval_crawler_expired_eval) {
                crawler_expired_eval(&active_crawler_mod, search, hv, i);
            } else if (active_crawler_mod.mod->eval_signature == eval_crawler_metadump_eval) {
                crawler_metadump_eval(&active_crawler_mod, search, hv, i);
            } else if (active_crawler_mod.mod->eval_signature == eval_crawler_mgdump_eval) {
                crawler_mgdump_eval(&active_crawler_mod, search, hv, i);
            }
    ...
    if (active_crawler_mod.mod != NULL) {
        if (active_crawler_mod.mod->finalize != NULL)
            if (active_crawler_mod.mod->finalize_signature == finalize_crawler_expired_finalize) {
                crawler_expired_finalize(&active_crawler_mod);
            } else if (active_crawler_mod.mod->finalize_signature == finalize_crawler_metadump_finalize) {
                crawler_metadump_finalize(&active_crawler_mod);
            } else if (active_crawler_mod.mod->finalize_signature == finalize_crawler_mgdump_finalize) {
                crawler_mgdump_finalize(&active_crawler_mod);
            }
...
int lru_crawler_start(uint8_t *ids, uint32_t remaining,
    ...
    if (!is_running) {
        ...
        if (active_crawler_mod.mod->init != NULL) {
            if (active_crawler_mod.mod->init_signature == init_crawler_expired_init) {
                crawler_expired_init(&active_crawler_mod, data);
            } else if (active_crawler_mod.mod->init_signature == init_crawler_metadump_init) {
                crawler_metadump_init(&active_crawler_mod, data);
            } else if (active_crawler_mod.mod->init_signature == init_crawler_mgdump_init) {
                crawler_mgdump_init(&active_crawler_mod, data);
            }
        }
