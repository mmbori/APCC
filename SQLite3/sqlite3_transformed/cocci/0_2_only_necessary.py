...
빌드에 필요한 파일 목록을 제외한 모든 .c, .h, .in 파일만 삭제 (다른 확장자는 보존)
"""
...
class NginxFileCleanup:
    ...
    def load_required_files(self):
        ...
        cc_count = sum(1 for f in self.required_files if f.suffix == '.in')
        other_count = len(self.required_files) - c_count - h_count - cc_count
        
        print(f"  - .c files: {c_count}")
        print(f"  - .h files: {h_count}")
        print(f"  - .in files: {cc_count}")
        if other_count > 0:
            print(f"  - other files: {other_count}")
    
    def find_all_source_files(self) -> List[Path]:
        """Nginx 루트 디렉토리에서 모든 .c, .h, .in 파일 찾기 (다른 확장자는 제외)"""
        print(f"\nScanning for .c, .h, and .in files in: {self.nginx_root}")
        
        all_files = []
        
        # .c, .h, .in 파일만 찾기 (다른 확장자는 무시)
        for ext in ['*.c', '*.h', '*.in']:
            all_files.extend(self.nginx_root.rglob(ext))
        
        print(f"Found {len(all_files)} total .c, .h, and .in files")
        
        # 타입별 통계
        c_count = sum(1 for f in all_files if f.suffix == '.c')
        h_count = sum(1 for f in all_files if f.suffix == '.h')
        cc_count = sum(1 for f in all_files if f.suffix == '.in')
        print(f"  - .c files: {c_count}")
        print(f"  - .h files: {h_count}")
        print(f"  - .in files: {cc_count}")
    ...
    def identify_files_to_delete(self):
        ...
        cc_count = sum(1 for f in self.files_to_delete if f.suffix == '.in')
        print(f"  - .c files to delete: {c_count}")
        print(f"  - .h files to delete: {h_count}")
        print(f"  - .in files to delete: {cc_count}")
    ...
    def print_summary(self):
        ...
        for dir_name, files in sorted(dir_groups.items()):
            ...
            cc_files = [f for f in files if f.suffix == '.in']
            
            print(f"\n{dir_name}/ ({len(files)} files: {len(c_files)} .c, {len(h_files)} .h, {len(cc_files)} .in)")
    ...
    def save_deletion_list(self, output_file: str = "files_to_delete.txt"):
        ...
        with open(output_file, 'w') as f:
            f.write("# Files to be deleted (.c, .h, and .in only)\n")
    ...
    def delete_files(self):
        ...
        if self.dry_run:
            ...
            print("="*70)
            print("\nNOTE: Only .c, .h, and .in files will be deleted.")
        ...
        print("\n" + "="*70)
        print("DELETING FILES (.c, .h, and .in only)")
        print("="*70)
    ...
    def run(self):
        ...
        print(f"Target: .c, .h, and .in files ONLY (other files preserved)")
        print()
        ...
        if not self.files_to_delete:
            print("\nNo files to delete. All .c, .h, and .in files are required.")
            return
...
def main():
    parser = argparse.ArgumentParser(
        description='Delete unnecessary .c, .h, and .in files from Nginx directory (preserves all other file types)',
...
Note:
  - Only .c, .h, and .in files will be deleted
  ...
  - Files listed in the input file are always kept
    ...
    if args.execute:
        print("\n" + "!"*70)
        print("WARNING: This will permanently delete .c, .h, and .in files!")
