...
#define TIFF_LAZYSTRILELOAD_ASKED                                              \
    ...
    int (*tif_fixuptags)(TIFF *);   /* called in TIFFReadDirectory */
    int (*tif_setupdecode)(TIFF *); /* called once before predecode */
    int (*tif_predecode)(TIFF *, uint16_t);    /* pre- row/strip/tile decoding */
    int (*tif_setupencode)(TIFF *); /* called once before preencode */
    int tif_encodestatus;
    int (*tif_preencode)(TIFF *, uint16_t);      /* pre- row/strip/tile encoding */
    int (*tif_postencode)(TIFF *);    /* post- row/strip/tile encoding */
    int (*tif_decoderow)(TIFF *tif, uint8_t *buf, tmsize_t size,
                              uint16_t sample);     /* scanline decoding routine */
    int (*tif_encoderow)(TIFF *tif, uint8_t *buf, tmsize_t size,
                              uint16_t sample);     /* scanline encoding routine */
    int (*tif_decodestrip)(TIFF *tif, uint8_t *buf, tmsize_t size,
                              uint16_t sample);   /* strip decoding routine */
    int (*tif_encodestrip)(TIFF *tif, uint8_t *buf, tmsize_t size,
                              uint16_t sample);   /* strip encoding routine */
    int (*tif_decodetile)(TIFF *tif, uint8_t *buf, tmsize_t size,
                              uint16_t sample);    /* tile decoding routine */
    int (*tif_encodetile)(TIFF *tif, uint8_t *buf, tmsize_t size,
                              uint16_t sample);    /* tile encoding routine */
    void (*tif_close)(TIFF *);         /* cleanup-on-close routine */
    int (*tif_seek)(TIFF *, uint32_t);          /* position within a strip routine */
    void (*tif_cleanup)(TIFF *);       /* cleanup state routine */
    uint32_t (*tif_defstripsize)(TIFF *, uint32_t); /* calculate/constrain strip size */
    void (*tif_deftilesize)(TIFF *, uint32_t *, uint32_t *);   /* calculate/constrain tile size */
    ...
    int (*tif_mapproc)(thandle_t, void **base, toff_t *size);     /* map file method */
    void (*tif_unmapproc)(thandle_t, void *base, toff_t size); /* unmap file method */
    /* input/output callback methods */
    thandle_t tif_clientdata;        /* callback parameter */
    tmsize_t (*tif_readproc)(thandle_t, void *, tmsize_t);  /* read method */
    tmsize_t (*tif_writeproc)(thandle_t, void *, tmsize_t); /* write method */
    toff_t (*tif_seekproc)(thandle_t, toff_t, int);       /* lseek method */
    int (*tif_closeproc)(thandle_t);     /* close method */
    toff_t (*tif_sizeproc)(thandle_t);       /* filesize method */
    /* post-decoding support */
    void (*tif_postdecode)(TIFF *tif, uint8_t *buf, tmsize_t size); /* post decoding routine */
    /* tag support */
    ...
    int (*tif_errorhandler)(TIFF *, void *user_data, const char *,
                                        const char *, va_list);
    void *tif_errorhandler_user_data;
    int (*tif_warnhandler)(TIFF *, void *user_data, const char *,
                                        const char *, va_list);
    ...
    int tif_fixuptags_signature;
    int tif_setupdecode_signature;
    int tif_predecode_signature;
    int tif_setupencode_signature;
    int tif_preencode_signature;
    int tif_postencode_signature;
    int tif_decoderow_signature;
    int tif_encoderow_signature;
    int tif_decodestrip_signature;
    int tif_encodestrip_signature;
    int tif_decodetile_signature;
    int tif_encodetile_signature;
    int tif_close_signature;
    int tif_seek_signature;
    int tif_cleanup_signature;
    int tif_defstripsize_signature;
    int tif_deftilesize_signature;
    int tif_mapproc_signature;
    int tif_unmapproc_signature;
    int tif_readproc_signature;
    int tif_writeproc_signature;
    int tif_seekproc_signature;
    int tif_closeproc_signature;
    int tif_sizeproc_signature;
    int tif_postdecode_signature;
    int tif_errorhandler_signature;
    int tif_warnhandler_signature;
};

struct TIFFOpenOptions
{
    int (*errorhandler)(TIFF *, void *user_data, const char *,
                                        const char *, va_list); /* may be NULL */
    void *errorhandler_user_data;      /* may be NULL */
    int (*warnhandler)(TIFF *, void *user_data, const char *,
                                        const char *, va_list);  /* may be NULL */
    ...
    int errorhandler_signature;
    int warnhandler_signature;
};


// declarations
uint64_t _tiffSeekProc(thandle_t fd, uint64_t off, int whence);
int _tiffCloseProc(thandle_t fd);
uint64_t _tiffSizeProc(thandle_t fd);
...
#define TIFFReadFile(tif, buf, size)                                           \
    _tiffReadProc((tif)->tif_clientdata, (buf), (size))
#define TIFFWriteFile(tif, buf, size)                                          \
    _tiffWriteProc((tif)->tif_clientdata, (buf), (size))
#define TIFFSeekFile(tif, off, whence)                                         \
    _tiffSeekProc((tif)->tif_clientdata, (off), (whence))
#define TIFFCloseFile(tif) _tiffCloseProc((tif)->tif_clientdata)
#define TIFFGetFileSize(tif) _tiffSizeProc((tif)->tif_clientdata)
#define TIFFMapFileContents(tif, paddr, psize)                                 \
    _tiffMapProc((tif)->tif_clientdata, (paddr), (psize))
#define TIFFUnmapFileContents(tif, addr, size)                                 \
    _tiffDummyUnmapProc((tif)->tif_clientdata, (addr), (size))
...
extern "C"
{
    ...
    extern void (*_TIFFwarningHandler)(const char *, const char *, va_list);
    extern void (*_TIFFerrorHandler)(const char *, const char *, va_list);
    extern void (*_TIFFwarningHandlerExt)(thandle_t, const char *, const char *,
                                        va_list);
    extern void (*_TIFFerrorHandlerExt)(thandle_t, const char *, const char *,
                                        va_list);
