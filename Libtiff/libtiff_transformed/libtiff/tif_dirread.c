...
int TIFFReadDirectory(TIFF *tif)
{
    ...
    switch (tif->tif_fixuptags_signature) {
    case tif_fixuptags_DumpFixupTags:
        DumpFixupTags(tif);
        break;
    case tif_fixuptags_Fax3FixupTags:
        Fax3FixupTags(tif);
        break;
    case tif_fixuptags_LZWFixupTags:
        LZWFixupTags(tif);
        break;
    case tif_fixuptags_LogLuvFixupTags:
        LogLuvFixupTags(tif);
        break;
    case tif_fixuptags_PixarLogFixupTags:
        PixarLogFixupTags(tif);
        break;
    case tif_fixuptags_ZIPFixupTags:
        ZIPFixupTags(tif);
        break;
    case tif_fixuptags__TIFFNoFixupTags:
        _TIFFNoFixupTags(tif);
        break;
    case tif_fixuptags__notConfigured:
        _notConfigured(tif);
        break;
    default:
...
int _TIFFCheckDirNumberAndOffset(TIFF *tif, tdir_t dirn, uint64_t diroff)
{
    ...
    if (tif->tif_map_dir_offset_to_number == NULL)
    {
        tif->tif_map_dir_offset_to_number = TIFFHashSetNew(
            hashFuncOffsetToNumber, fnHashFunc_hashFuncOffsetToNumber, equalFuncOffsetToNumber, fnEqualFunc_equalFuncOffsetToNumber, free, fnFreeEltFunc_free);
        if (tif->tif_map_dir_offset_to_number == NULL)
        {
    ...
    if (tif->tif_map_dir_number_to_offset == NULL)
    {
        ...
        tif->tif_map_dir_number_to_offset = TIFFHashSetNew(
            hashFuncNumberToOffset, fnHashFunc_hashFuncNumberToOffset, equalFuncNumberToOffset, fnEqualFunc_equalFuncNumberToOffset, NULL, fnFreeEltFunc_NULL);
        if (tif->tif_map_dir_number_to_offset == NULL)
        {
            TIFFErrorExtR(tif, "_TIFFCheckDirNumberAndOffset",
                          "Not enough memory");
