...
int TIFFWriteScanline(TIFF *tif, void *buf, uint32_t row, uint16_t sample)
{
    ...
    default:
        ...
        int tmp3;
        switch (tif->tif_preencode_signature) {
    case tif_preencode_Fax3PreEncode:
        tmp3 = Fax3PreEncode(tif, sample);
        break;
    case tif_preencode_LZWPreEncode:
        tmp3 = LZWPreEncode(tif, sample);
        break;
    case tif_preencode_PackBitsPreEncode:
        tmp3 = PackBitsPreEncode(tif, sample);
        break;
    case tif_preencode_PixarLogPreEncode:
        tmp3 = PixarLogPreEncode(tif, sample);
        break;
    case tif_preencode_ZIPPreEncode:
        tmp3 = ZIPPreEncode(tif, sample);
        break;
    case tif_preencode__TIFFNoPreCode:
        tmp3 = _TIFFNoPreCode(tif, sample);
        break;
    default:
        invalidFunctionWasCalled();
        break;
    ...
    // status = (*tif->tif_encoderow)(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
    switch (tif->tif_encoderow_signature) {
    case tif_encoderow_DumpModeEncode:
        status = DumpModeEncode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_Fax3Encode:
        status = Fax3Encode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_Fax4Encode:
        status = Fax4Encode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_LZWEncode:
        status = LZWEncode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_LogL16Encode:
        status = LogL16Encode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_LogLuvEncode24:
        status = LogLuvEncode24(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_LogLuvEncode32:
        status = LogLuvEncode32(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_PackBitsEncode:
        status = PackBitsEncode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_PixarLogEncode:
        status = PixarLogEncode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_PredictorEncodeRow:
        status = PredictorEncodeRow(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow_ZIPEncode:
        status = ZIPEncode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    case tif_encoderow__TIFFNoRowEncode:
        status = _TIFFNoRowEncode(tif, (uint8_t *)buf, tif->tif_scanlinesize, sample);
        break;
    default:
        invalidFunctionWasCalled();
        break;
                               ...
                               tmsize_t cc)
{
    ...
    if (td->td_compression == COMPRESSION_NONE)
    {
        ...
        switch (tif->tif_postdecode_signature) {
        case tif_postdecode__TIFFNoPostDecode:
            _TIFFNoPostDecode(tif, (uint8_t *)data, cc);
            break;
        case tif_postdecode__TIFFSwab16BitData:
            _TIFFSwab16BitData(tif, (uint8_t *)data, cc);
            break;
        case tif_postdecode__TIFFSwab24BitData:
            _TIFFSwab24BitData(tif, (uint8_t *)data, cc);
            break;
        case tif_postdecode__TIFFSwab32BitData:
            _TIFFSwab32BitData(tif, (uint8_t *)data, cc);
            break;
        case tif_postdecode__TIFFSwab64BitData:
            _TIFFSwab64BitData(tif, (uint8_t *)data, cc);
            break;
        default:
            invalidFunctionWasCalled();
            break;
    ...
    int tmp;
            switch (tif->tif_postencode_signature) {
                case tif_postencode_Fax3PostEncode:
                    tmp = Fax3PostEncode(tif);
                    break;
                case tif_postencode_Fax4PostEncode:
                    tmp = Fax4PostEncode(tif);
                    break;
                case tif_postencode_LZWPostEncode:
                    tmp = LZWPostEncode(tif);
                    break;
                case tif_postencode_PackBitsPostEncode:
                    tmp = PackBitsPostEncode(tif);
                    break;
                case tif_postencode_PixarLogPostEncode:
                    tmp = PixarLogPostEncode(tif);
                    break;
                case tif_postencode_ZIPPostEncode:
                    tmp = ZIPPostEncode(tif);
                    break;
                case tif_postencode__TIFFtrue:
                    tmp = _TIFFtrue(tif);
                    break;
                default:
                    invalidFunctionWasCalled();
...
tmsize_t TIFFWriteEncodedTile(TIFF *tif, uint32_t tile, void *data, tmsize_t cc)
{
    ...
    int tmp2;
    switch (tif->tif_encodetile_signature) {
    case tif_encodetile_DumpModeEncode:
        tmp2 = DumpModeEncode(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_Fax3Encode:
        tmp2 = Fax3Encode(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_Fax4Encode:
        tmp2 = Fax4Encode(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_LZWEncode:
        tmp2 = LZWEncode(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_LogLuvEncodeTile:
        tmp2 = LogLuvEncodeTile(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_PackBitsEncodeChunk:
        tmp2 = PackBitsEncodeChunk(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_PixarLogEncode:
        tmp2 = PixarLogEncode(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_PredictorEncodeTile:
        tmp2 = PredictorEncodeTile(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile_ZIPEncode:
        tmp2 = ZIPEncode(tif, (uint8_t *)data, cc, sample);
        break;
    case tif_encodetile__TIFFNoTileEncode:
        tmp2 = _TIFFNoTileEncode(tif, (uint8_t *)data, cc, sample);
        break;
    default:
        invalidFunctionWasCalled();
        break;
