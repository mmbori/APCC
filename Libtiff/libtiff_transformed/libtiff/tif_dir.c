        ...
        case TIFFTAG_BITSPERSAMPLE:
            ...
            if (tif->tif_flags & TIFF_SWAB)
            {
                if (td->td_bitspersample == 8) {
                    ...
                    tif->tif_postdecode_signature = tif_postdecode__TIFFSwab16BitData;
                }
                else if (td->td_bitspersample == 24) {
                    tif->tif_postdecode = _TIFFSwab24BitData;
                    tif->tif_postdecode_signature = tif_postdecode__TIFFSwab24BitData;
                }
                else if (td->td_bitspersample == 32) {
                    tif->tif_postdecode = _TIFFSwab32BitData;
                    tif->tif_postdecode_signature = tif_postdecode__TIFFSwab32BitData;
                }
                else if (td->td_bitspersample == 64) {
                    tif->tif_postdecode = _TIFFSwab64BitData;
                    tif->tif_postdecode_signature = tif_postdecode__TIFFSwab64BitData;
                }
        ...
        case TIFFTAG_COMPRESSION:
            ...
            if (TIFFFieldSet(tif, FIELD_COMPRESSION))
            {
                if ((uint32_t)td->td_compression == v)
                    break;
                // (*tif->tif_cleanup)(tif);
                switch (tif->tif_cleanup_signature) {
    case tif_cleanup_Fax3Cleanup:
        Fax3Cleanup(tif);
        break;
    case tif_cleanup_LZWCleanup:
        LZWCleanup(tif);
        break;
    case tif_cleanup_LogLuvCleanup:
        LogLuvCleanup(tif);
        break;
    case tif_cleanup_PixarLogCleanup:
        PixarLogCleanup(tif);
        break;
    case tif_cleanup_ZIPCleanup:
        ZIPCleanup(tif);
        break;
    case tif_cleanup__TIFFvoid:
        _TIFFvoid(tif);
        break;
    default:
...
int TIFFDefaultDirectory(TIFF *tif)
{
    ...
    tif->tif_tagmethods.vsetfield_signature = vsetfield__TIFFVSetField;
    tif->tif_tagmethods.vgetfield = _TIFFVGetField;
    tif->tif_tagmethods.vgetfield_signature = vgetfield__TIFFVGetField;
