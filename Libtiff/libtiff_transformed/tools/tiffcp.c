...
static int tiffcp(TIFF *in, TIFF *out)
{
    ...
    int (*cf)(TIFF *in, TIFF *out, uint32_t l, uint32_t w,
                        uint16_t samplesperpixel);
...
#define subtract(bits)                                                         \
    ...
    {                                                                          \
        uint##bits##_t *image = (uint##bits##_t *)i;                           \
...
DECLAREcpFunc(cpBiasedContig2Contig)
{
    if (spp == 1)
    {
            ...
            imagewidth == biasWidth)
        {
            ...
            void (*subtractLine)(void *image, void *bias, uint32_t pixels);
...
static int cpImage(TIFF *in, TIFF *out, int (*fin)(TIFF *, uint8_t *, uint32_t, uint32_t, tsample_t), int fin_signature, int (*fout)(TIFF *, uint8_t *, uint32_t, uint32_t, tsample_t),
                   int fout_signature, uint32_t imagelength, uint32_t imagewidth, tsample_t spp)
{
    int status = 0;
    tdata_t buf = NULL;
        ...
        ((TIFF_TMSIZE_T_MAX / (tmsize_t)imagelength) > scanlinesize))
    {
        ...
        {
            int tmp;
            switch (fin_signature)
            {
            case fin_readSeparateTilesIntoBuffer :
                tmp = readSeparateTilesIntoBuffer(in, (uint8_t *)buf, imagelength, imagewidth, spp);
                break;
            case fin_readContigTilesIntoBuffer :
                tmp = readContigTilesIntoBuffer(in, (uint8_t *)buf, imagelength, imagewidth, spp);
                break;
            case fin_readContigStripsIntoBuffer :
                tmp = readContigStripsIntoBuffer(in, (uint8_t *)buf, imagelength, imagewidth, spp);
                break;           
            case fin_readSeparateStripsIntoBuffer :
                tmp = readSeparateStripsIntoBuffer(in, (uint8_t *)buf, imagelength, imagewidth, spp);
                break;    
            default:
            ...
            if (tmp)
            {
                ...
                switch (fout_signature)
                {
                case fout_writeBufferToContigTiles :
                    status = writeBufferToContigTiles(out, (uint8_t *)buf, imagelength, imagewidth, spp);
                    break;
                case fout_writeBufferToSeparateTiles :
                    status = writeBufferToSeparateTiles(out, (uint8_t *)buf, imagelength, imagewidth, spp);
                    break;
                case fout_writeBufferToContigStrips :
                    status = writeBufferToContigStrips(out, (uint8_t *)buf, imagelength, imagewidth, spp);
                    break;            
                case fout_writeBufferToSeparateStrips :
                    status = writeBufferToSeparateStrips(out, (uint8_t *)buf, imagelength, imagewidth, spp);
                    break;  
                default:
                    invalidFunctionWasCalled();
                    break;
...
{
    return cpImage(in, out, readContigStripsIntoBuffer, fin_readContigStripsIntoBuffer,
                   writeBufferToContigTiles, fout_writeBufferToContigTiles, imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readContigStripsIntoBuffer, fin_readContigStripsIntoBuffer,
                   writeBufferToSeparateTiles, fout_writeBufferToSeparateTiles , imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readSeparateStripsIntoBuffer, fin_readSeparateStripsIntoBuffer,
                   writeBufferToContigTiles, fout_writeBufferToContigTiles , imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readContigTilesIntoBuffer,  fin_readContigTilesIntoBuffer , writeBufferToContigTiles,
                   fout_writeBufferToContigTiles , imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readContigTilesIntoBuffer, fin_readContigTilesIntoBuffer,
                   writeBufferToSeparateTiles, fout_writeBufferToSeparateTiles , imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readSeparateTilesIntoBuffer, fin_readSeparateTilesIntoBuffer,
                   writeBufferToContigTiles, fout_writeBufferToContigTiles , imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readContigTilesIntoBuffer, fin_readContigTilesIntoBuffer,
                   writeBufferToContigStrips, fout_writeBufferToContigStrips , imagelength, imagewidth, spp);
}
...
{
    return cpImage(in, out, readContigTilesIntoBuffer, fin_readContigTilesIntoBuffer,
                   writeBufferToSeparateStrips, fout_writeBufferToSeparateStrips , imagelength, imagewidth, spp);
}
