...
{	OGG_PRIVATE* odata = psf->container_data ;
	...
	{	/* Call this here so it only gets called once, so no memory is leaked. */
		...
		psf->read_short_signature = read_short_opcm_read_s;
		psf->read_int = opcm_read_i;
		psf->read_int_signature = read_int_opcm_read_i;
		psf->read_float = opcm_read_f;
		psf->read_float_signature = read_float_opcm_read_f;
		psf->read_double = opcm_read_d;
		psf->read_double_signature = read_double_opcm_read_d;
		psf->sf.frames		= opcm_length (psf) ;
#endif
		} ;

	psf->codec_close = opcm_close;
	psf->codec_close_signature = codec_close_opcm_close;
	...
	if (psf->file.mode == SFM_WRITE)
	{
		...
		psf->write_header_signature = write_header_opcm_write_header;
		psf->write_short = opcm_write_s;
		psf->write_short_signature = write_short_opcm_write_s;
		psf->write_int = opcm_write_i;
		psf->write_int_signature = write_int_opcm_write_i;
		psf->write_float = opcm_write_f;
		psf->write_float_signature = write_float_opcm_write_f;
		psf->write_double = opcm_write_d;
		psf->write_double_signature = write_double_opcm_write_d;
#endif
...
#if 0
	psf->seek = opcm_seek;
	psf->seek_signature = seek_opcm_seek;
	psf->command = opcm_command;
	psf->command_signature = command_opcm_command;
#endif
...
int opcm_close(SF_PRIVATE * UNUSED(psf))
{
