...
sf_count_t nms_adpcm_write_s(SF_PRIVATE *psf, const short *ptr,
			     sf_count_t len) ;
sf_count_t nms_adpcm_write_i(SF_PRIVATE *psf, const int *ptr, sf_count_t len) ;
sf_count_t nms_adpcm_write_f(SF_PRIVATE *psf, const float *ptr,
			     sf_count_t len) ;
sf_count_t nms_adpcm_write_d(SF_PRIVATE *psf, const double *ptr,
			     sf_count_t len) ;
...
{	NMS_ADPCM_PRIVATE	*pnms ;
	...
	psf->read_short_signature = read_short_nms_adpcm_read_s;
		psf->read_int = nms_adpcm_read_i;
		psf->read_int_signature = read_int_nms_adpcm_read_i;
		psf->read_float = nms_adpcm_read_f;
		psf->read_float_signature = read_float_nms_adpcm_read_f;
		psf->read_double = nms_adpcm_read_d;
		psf->read_double_signature = read_double_nms_adpcm_read_d;
		}
	else if (psf->file.mode == SFM_WRITE)
	{	psf->write_short = nms_adpcm_write_s;
	psf->write_short_signature = write_short_nms_adpcm_write_s;
		psf->write_int = nms_adpcm_write_i;
		psf->write_int_signature = write_int_nms_adpcm_write_i;
		psf->write_float = nms_adpcm_write_f;
		psf->write_float_signature = write_float_nms_adpcm_write_f;
		psf->write_double = nms_adpcm_write_d;
		psf->write_double_signature = write_double_nms_adpcm_write_d;
		} ;
	...
	psf->codec_close_signature = codec_close_nms_adpcm_close;
	psf->seek = nms_adpcm_seek;
	psf->seek_signature = seek_nms_adpcm_seek;
	...
	{	if (pnms->sample_curr && pnms->sample_curr < NMS_SAMPLES_PER_BLOCK)
		...
		if (psf->write_header)
			...
			case write_header_wve_write_header:
			    wve_write_header(psf, SF_FALSE);
			    break;
			case write_header_xi_write_header:
			    xi_write_header(psf, SF_FALSE);
			    break;
			default:
