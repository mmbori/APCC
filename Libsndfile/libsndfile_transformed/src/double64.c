...
sf_count_t replace_write_s2d(SF_PRIVATE *psf, const short *ptr,
			     sf_count_t len) ;
sf_count_t replace_write_i2d(SF_PRIVATE *psf, const int *ptr, sf_count_t len) ;
sf_count_t replace_write_f2d(SF_PRIVATE *psf, const float *ptr,
			     sf_count_t len) ;
...
{	static int double64_caps ;
	...
	{	switch (psf->endian + double64_caps)
		{	case (SF_ENDIAN_BIG + DOUBLE_CAN_RW_BE) :
					...
					psf->read_short_signature = read_short_host_read_d2s;
					psf->read_int = host_read_d2i;
					psf->read_int_signature = read_int_host_read_d2i;
					psf->read_float = host_read_d2f;
					psf->read_float_signature = read_float_host_read_d2f;
					psf->read_double = host_read_d;
					psf->read_double_signature = read_double_host_read_d;
					break ;
			...
			case (SF_ENDIAN_BIG + DOUBLE_BROKEN_BE) :
					...
					psf->read_short_signature = read_short_replace_read_d2s;
					psf->read_int = replace_read_d2i;
					psf->read_int_signature = read_int_replace_read_d2i;
					psf->read_float = replace_read_d2f;
					psf->read_float_signature = read_float_replace_read_d2f;
					psf->read_double = replace_read_d;
					psf->read_double_signature = read_double_replace_read_d;
					break ;
	...
	{	switch (psf->endian + double64_caps)
		{	case (SF_ENDIAN_LITTLE + DOUBLE_CAN_RW_LE) :
					...
					psf->write_short_signature = write_short_host_write_s2d;
					psf->write_int = host_write_i2d;
					psf->write_int_signature = write_int_host_write_i2d;
					psf->write_float = host_write_f2d;
					psf->write_float_signature = write_float_host_write_f2d;
					psf->write_double = host_write_d;
					psf->write_double_signature = write_double_host_write_d;
					break ;
			...
			case (SF_ENDIAN_LITTLE + DOUBLE_BROKEN_LE) :
					...
					psf->write_short_signature = write_short_replace_write_s2d;
					psf->write_int = replace_write_i2d;
					psf->write_int_signature = write_int_replace_write_i2d;
					psf->write_float = replace_write_f2d;
					psf->write_float_signature = write_float_replace_write_f2d;
					psf->write_double = replace_write_d;
					psf->write_double_signature = write_double_replace_write_d;
					break ;
...
{	BUF_UNION	ubuf ;
	...
	{	if (len < bufferlen)
		...
		if (psf->add_clipping) {
		d2s_clip_array (ubuf.dbuf, readcount, ptr + total, scale) ;
		}
		else {
...
{	BUF_UNION	ubuf ;
	...
	{	if (len < bufferlen)
		...
		// convert (ubuf.dbuf, readcount, ptr + total, scale) ;
		if (psf->add_clipping) {
		d2i_clip_array (ubuf.dbuf, readcount, ptr + total, scale) ;
		}
		else {
