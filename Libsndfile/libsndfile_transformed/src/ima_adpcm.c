...
{	int error ;
	...
	psf->codec_close_signature = codec_close_ima_close;
	psf->seek = wavlike_ima_seek;
	psf->seek_signature = seek_wavlike_ima_seek;
...
{	int error ;
	...
	psf->seek_signature = seek_aiff_ima_seek;
	...
	{	/*	If a block has been partially assembled, write it out
		...
		*/
		if (pima->samplecount && pima->samplecount < pima->samplesperblock)
			switch (pima->encode_block_signature) {
			case encode_block_aiff_ima_encode_block:
			    aiff_ima_encode_block(psf, pima);
			    break;
			case encode_block_wavlike_ima_encode_block:
			    wavlike_ima_encode_block(psf, pima);
			    break;
			default:
	...
	*/
	...
	{	case SF_FORMAT_WAV :
		case SF_FORMAT_W64 :
				...
				pima->decode_block_signature = decode_block_wavlike_ima_decode_block;
		...
		case SF_FORMAT_AIFF :
				...
				pima->decode_block_signature = decode_block_aiff_ima_decode_block;
	...
	switch (pima->decode_block_signature) {
	case decode_block_aiff_ima_decode_block:
	    aiff_ima_decode_block(psf, pima);
	    break;
	case decode_block_wavlike_ima_decode_block:
	    wavlike_ima_decode_block(psf, pima);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
	...
	psf->read_short_signature = read_short_ima_read_s;
	psf->read_int = ima_read_i;
	psf->read_int_signature = read_int_ima_read_i;
	psf->read_float = ima_read_f;
	psf->read_float_signature = read_float_ima_read_f;
	psf->read_double = ima_read_d;
	psf->read_double_signature = read_double_ima_read_d;
...
{	IMA_ADPCM_PRIVATE	*pima ;
	...
	{	case SF_FORMAT_WAV :
		case SF_FORMAT_W64 :
				...
				pima->encode_block_signature = encode_block_wavlike_ima_encode_block;
				break ;

		case SF_FORMAT_AIFF :
				pima->encode_block = aiff_ima_encode_block;
				pima->encode_block_signature = encode_block_aiff_ima_encode_block;
				break ;
	...
	psf->write_short_signature = write_short_ima_write_s;
	psf->write_int = ima_write_i;
	psf->write_int_signature = write_int_ima_write_i;
	psf->write_float = ima_write_f;
	psf->write_float_signature = write_float_ima_write_f;
	psf->write_double = ima_write_d;
	psf->write_double_signature = write_double_ima_write_d;
