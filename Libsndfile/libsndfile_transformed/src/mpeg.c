...
int mpeg_write_header(SF_PRIVATE *psf, int UNUSED(calc_length))
{
	if (psf->have_written)
		return 0 ;
	...
	{	switch (SF_CODEC (psf->sf.format))
		{	case SF_FORMAT_MPEG_LAYER_III :
				...
				{	/* ID3 support */
					...
					psf->write_header_signature = write_header_mpeg_write_header;
					} ;
				break ;
...
{	int error ;
	...
	psf->command_signature = command_mpeg_command;
