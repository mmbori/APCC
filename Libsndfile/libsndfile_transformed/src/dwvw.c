...
{	DWVW_PRIVATE	*pdwvw ;
	...
	psf->read_short_signature = read_short_dwvw_read_s;
		psf->read_int = dwvw_read_i;
		psf->read_int_signature = read_int_dwvw_read_i;
		psf->read_float = dwvw_read_f;
		psf->read_float_signature = read_float_dwvw_read_f;
		psf->read_double = dwvw_read_d;
		psf->read_double_signature = read_double_dwvw_read_d;
		} ;

	if (psf->file.mode == SFM_WRITE)
	{	psf->write_short = dwvw_write_s;
	psf->write_short_signature = write_short_dwvw_write_s;
		psf->write_int = dwvw_write_i;
		psf->write_int_signature = write_int_dwvw_write_i;
		psf->write_float = dwvw_write_f;
		psf->write_float_signature = write_float_dwvw_write_f;
		psf->write_double = dwvw_write_d;
		psf->write_double_signature = write_double_dwvw_write_d;
		} ;

	psf->codec_close = dwvw_close;
	psf->codec_close_signature = codec_close_dwvw_close;
	psf->seek = dwvw_seek;
	psf->seek_signature = seek_dwvw_seek;
	psf->byterate = dwvw_byterate;
	psf->byterate_signature = byterate_dwvw_byterate;
...
sf_count_t dwvw_seek(SF_PRIVATE *psf, int UNUSED(mode), sf_count_t offset)
{	DWVW_PRIVATE *pdwvw ;
