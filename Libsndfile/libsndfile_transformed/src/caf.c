...
SF_CHUNK_ITERATOR *  caf_next_chunk_iterator(SF_PRIVATE *psf,
					     SF_CHUNK_ITERATOR * iterator) ;
int caf_get_chunk_size(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
		       SF_CHUNK_INFO * chunk_info) ;
int caf_get_chunk_data(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
		       SF_CHUNK_INFO * chunk_info) ;
...
{	CAF_PRIVATE * pcaf ;
	...
	{	if ((error = caf_read_header (psf)))
		...
		psf->next_chunk_iterator_signature = next_chunk_iterator_caf_next_chunk_iterator;
		psf->get_chunk_size = caf_get_chunk_size;
		psf->get_chunk_size_signature = get_chunk_size_caf_get_chunk_size;
		psf->get_chunk_data = caf_get_chunk_data;
		psf->get_chunk_data_signature = get_chunk_data_caf_get_chunk_data;
		} ;
	...
	{	if (psf->is_pipe)
		...
		*/
		...
		psf->write_header_signature = write_header_caf_write_header;
		psf->set_chunk = caf_set_chunk;
		psf->set_chunk_signature = set_chunk_caf_set_chunk;
		} ;

	psf->container_close = caf_close;
	psf->container_close_signature = container_close_caf_close;
	psf->command = caf_command;
	psf->command_signature = command_caf_command;
...
int caf_command(SF_PRIVATE * psf, int command, void * UNUSED(data),
		int UNUSED(datasize))
{	CAF_PRIVATE	*pcaf ;
