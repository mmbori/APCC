...
sf_count_t flac_write_s2flac(SF_PRIVATE *psf, const short *ptr,
			     sf_count_t len) ;
sf_count_t flac_write_i2flac(SF_PRIVATE *psf, const int *ptr, sf_count_t len) ;
sf_count_t flac_write_f2flac(SF_PRIVATE *psf, const float *ptr,
			     sf_count_t len) ;
sf_count_t flac_write_d2flac(SF_PRIVATE *psf, const double *ptr,
			     sf_count_t len) ;
...
{	int		subformat ;
	...
	{	if ((SF_CONTAINER (psf->sf.format)) != SF_FORMAT_FLAC)
		...
		*/
		...
		psf->write_header_signature = write_header_flac_write_header;
		} ;
	...
	psf->container_close_signature = container_close_flac_close;
	psf->seek = flac_seek;
	psf->seek_signature = seek_flac_seek;
	psf->byterate = flac_byterate;
	psf->byterate_signature = byterate_flac_byterate;

	psf->command = flac_command;
	psf->command_signature = command_flac_command;
...
flac_init (SF_PRIVATE *psf)
{
	...
	psf->read_short_signature = read_short_flac_read_flac2s;
		psf->read_int = flac_read_flac2i;
		psf->read_int_signature = read_int_flac_read_flac2i;
		psf->read_float = flac_read_flac2f;
		psf->read_float_signature = read_float_flac_read_flac2f;
		psf->read_double = flac_read_flac2d;
		psf->read_double_signature = read_double_flac_read_flac2d;
		} ;

	if (psf->file.mode == SFM_WRITE)
	{	psf->write_short = flac_write_s2flac;
	psf->write_short_signature = write_short_flac_write_s2flac;
		psf->write_int = flac_write_i2flac;
		psf->write_int_signature = write_int_flac_write_i2flac;
		psf->write_float = flac_write_f2flac;
		psf->write_float_signature = write_float_flac_write_f2flac;
		psf->write_double = flac_write_d2flac;
		psf->write_double_signature = write_double_flac_write_d2flac;
		} ;
...
{	FLAC_PRIVATE* pflac = (FLAC_PRIVATE*) psf->codec_data ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int) len ;
		...
		switch (SF_CODEC (psf->sf.format))
	{	case SF_FORMAT_PCM_S8 :
		s2flac8_array (ptr + total, buffer, writecount) ;
			break ;
		case SF_FORMAT_PCM_16 :
		s2flac16_array (ptr + total, buffer, writecount) ;
			break ;
		case SF_FORMAT_PCM_24 :
		s2flac24_array (ptr + total, buffer, writecount) ;
			break ;
		default :
			return -1 ;
		} ;
...
{	FLAC_PRIVATE* pflac = (FLAC_PRIVATE*) psf->codec_data ;
	...
	{	writecount = (len >= bufferlen) ? bufferlen : (int) len ;
		...
		switch (SF_CODEC (psf->sf.format))
	{	case SF_FORMAT_PCM_S8 :
		i2flac8_array (ptr + total, buffer, writecount) ;
			break ;
		case SF_FORMAT_PCM_16 :
		i2flac16_array (ptr + total, buffer, writecount) ;
			break ;
		case SF_FORMAT_PCM_24 :
		i2flac24_array (ptr + total, buffer, writecount) ;
			break ;
		default :
			return -1 ;
		} ;
...
sf_count_t flac_seek(SF_PRIVATE *psf, int UNUSED(mode), sf_count_t offset)
{	FLAC_PRIVATE* pflac = (FLAC_PRIVATE*) psf->codec_data ;
