...
{	OGG_PRIVATE* odata = psf->container_data ;
	...
	{	/* Call this here so it only gets called once, so no memory is leaked. */
		...
		psf->read_short_signature = read_short_spx_read_s;
		psf->read_int = spx_read_i;
		psf->read_int_signature = read_int_spx_read_i;
		psf->read_float = spx_read_f;
		psf->read_float_signature = read_float_spx_read_f;
		psf->read_double = spx_read_d;
		psf->read_double_signature = read_double_spx_read_d;
		psf->sf.frames		= spx_length (psf) ;
#endif
		} ;

	psf->codec_close = spx_close;
	psf->codec_close_signature = codec_close_spx_close;
	...
	if (psf->file.mode == SFM_WRITE)
	{
		...
		psf->write_header_signature = write_header_spx_write_header;
		psf->write_short = spx_write_s;
		psf->write_short_signature = write_short_spx_write_s;
		psf->write_int = spx_write_i;
		psf->write_int_signature = write_int_spx_write_i;
		psf->write_float = spx_write_f;
		psf->write_float_signature = write_float_spx_write_f;
		psf->write_double = spx_write_d;
		psf->write_double_signature = write_double_spx_write_d;
#endif
...
#if 0
	psf->seek = spx_seek;
	psf->seek_signature = seek_spx_seek;
	psf->command = spx_command;
	psf->command_signature = command_spx_command;
#endif
