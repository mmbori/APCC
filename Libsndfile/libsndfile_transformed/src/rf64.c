...
int rf64_command(SF_PRIVATE *psf, int command, void * UNUSED(data),
		 int datasize) ;

int rf64_set_chunk(SF_PRIVATE *psf, const SF_CHUNK_INFO * chunk_info) ;
SF_CHUNK_ITERATOR *  rf64_next_chunk_iterator(SF_PRIVATE *psf,
					      SF_CHUNK_ITERATOR * iterator) ;
int rf64_get_chunk_size(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
			SF_CHUNK_INFO * chunk_info) ;
int rf64_get_chunk_data(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
			SF_CHUNK_INFO * chunk_info) ;
...
{	WAVLIKE_PRIVATE *wpriv ;
	...
	{	if ((error = rf64_read_header (psf, &blockalign, &framesperblock)) != 0)
		...
		psf->next_chunk_iterator_signature = next_chunk_iterator_rf64_next_chunk_iterator;
		psf->get_chunk_size = rf64_get_chunk_size;
		psf->get_chunk_size_signature = get_chunk_size_rf64_get_chunk_size;
		psf->get_chunk_data = rf64_get_chunk_data;
		psf->get_chunk_data_signature = get_chunk_data_rf64_get_chunk_data;
		} ;
	...
	{	if (psf->is_pipe)
		...
		psf->write_header_signature = write_header_rf64_write_header;
		psf->set_chunk = rf64_set_chunk;
		psf->set_chunk_signature = set_chunk_rf64_set_chunk;
		} ;

	psf->container_close = rf64_close;
	psf->container_close_signature = container_close_rf64_close;
	psf->command = rf64_command;
	psf->command_signature = command_rf64_command;
