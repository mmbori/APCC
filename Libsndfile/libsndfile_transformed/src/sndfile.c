...
{	SF_PRIVATE *psf = (SF_PRIVATE *) sndfile ;
	...
	{	case SFC_SET_NORM_FLOAT :
		...
		case SFC_SET_ADD_PEAK_CHUNK :
			...
			if (psf->write_header)
				switch (psf->write_header_signature) {
				case write_header_aiff_write_header:
				    aiff_write_header(psf, SF_TRUE);
				    break;
				case write_header_au_write_header:
				    au_write_header(psf, SF_TRUE);
				    break;
				case write_header_avr_write_header:
				    avr_write_header(psf, SF_TRUE);
				    break;
				case write_header_caf_write_header:
				    caf_write_header(psf, SF_TRUE);
				    break;
				case write_header_htk_write_header:
				    htk_write_header(psf, SF_TRUE);
				    break;
				case write_header_ircam_write_header:
				    ircam_write_header(psf, SF_TRUE);
				    break;
				case write_header_mat4_write_header:
				    mat4_write_header(psf, SF_TRUE);
				    break;
				case write_header_mat5_write_header:
				    mat5_write_header(psf, SF_TRUE);
				    break;
				case write_header_mpc2k_write_header:
				    mpc2k_write_header(psf, SF_TRUE);
				    break;
				case write_header_nist_write_header:
				    nist_write_header(psf, SF_TRUE);
				    break;
				case write_header_paf_write_header:
				    paf_write_header(psf, SF_TRUE);
				    break;
				case write_header_pvf_write_header:
				    pvf_write_header(psf, SF_TRUE);
				    break;
				case write_header_rf64_write_header:
				    rf64_write_header(psf, SF_TRUE);
				    break;
				case write_header_sds_write_header:
				    sds_write_header(psf, SF_TRUE);
				    break;
				case write_header_svx_write_header:
				    svx_write_header(psf, SF_TRUE);
				    break;
				case write_header_voc_write_header:
				    voc_write_header(psf, SF_TRUE);
				    break;
				case write_header_w64_write_header:
				    w64_write_header(psf, SF_TRUE);
				    break;
				case write_header_wav_write_header:
				    wav_write_header(psf, SF_TRUE);
				    break;
				case write_header_wve_write_header:
				    wve_write_header(psf, SF_TRUE);
				    break;
				case write_header_xi_write_header:
				    xi_write_header(psf, SF_TRUE);
				    break;
				default:
		...
		case SFC_SET_CHANNEL_MAP_INFO :
			...
			*/
			if (psf->command) {
				switch (psf->command_signature) {
				case command_aiff_command:
				    return aiff_command(psf, command, NULL, 0);
				case command_caf_command:
				    return caf_command(psf, command, NULL, 0);
				case command_rf64_command:
				    return rf64_command(psf, command, NULL, 0);
				case command_wav_command:
				    return wav_command(psf, command, NULL, 0);
				default:
				    invalidFunctionWasCalled();
				}
			}
		...
		default :
			...
			if (psf->command) {
				switch (psf->command_signature) {
				case command_aiff_command:
				    return aiff_command(psf, command, data, datasize);
				case command_caf_command:
				    return caf_command(psf, command, data, datasize);
				case command_rf64_command:
				    return rf64_command(psf, command, data, datasize);
				case command_wav_command:
				    return wav_command(psf, command, data, datasize);
				default:
				    invalidFunctionWasCalled();
				}
			}
...
{	SF_PRIVATE 	*psf ;
	...
	if (psf->byterate)
		switch (psf->byterate_signature) {
		case byterate_alac_byterate:
		    return alac_byterate(psf);
		case byterate_dwvw_byterate:
		    return dwvw_byterate(psf);
		case byterate_sds_byterate:
		    return sds_byterate(psf);
		default:
		    invalidFunctionWasCalled();
		}
...
{	SF_PRIVATE 	*psf ;
	...
	if (psf->last_op != SFM_READ) {	
		...
		switch (psf->seek_signature) {
		case seek_aiff_ima_seek:
		    tmp = aiff_ima_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_alac_seek:
		    tmp = alac_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_dpcm_seek:
		    tmp = dpcm_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_dwvw_seek:
		    tmp = dwvw_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_g72x_seek:
		    tmp = g72x_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_gsm610_seek:
		    tmp = gsm610_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_interleave_seek:
		    tmp = interleave_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_msadpcm_seek:
		    tmp = msadpcm_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_nms_adpcm_seek:
		    tmp = nms_adpcm_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_paf24_seek:
		    tmp = paf24_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_psf_default_seek:
		    tmp = psf_default_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_sds_seek:
		    tmp = sds_seek(psf, SFM_READ, psf->read_current);
		    break;
		case seek_wavlike_ima_seek:
		    tmp = wavlike_ima_seek(psf, SFM_READ, psf->read_current);
		    break;
		default:
		    invalidFunctionWasCalled();
		    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_short_signature) {
	case read_short_alac_read_s:
	    count = alac_read_s(psf, ptr, len);
	    break;
	case read_short_alaw_read_alaw2s:
	    count = alaw_read_alaw2s(psf, ptr, len);
	    break;
	case read_short_dither_read_short:
	    count = dither_read_short(psf, ptr, len);
	    break;
	case read_short_dpcm_read_dles2s:
	    count = dpcm_read_dles2s(psf, ptr, len);
	    break;
	case read_short_dpcm_read_dsc2s:
	    count = dpcm_read_dsc2s(psf, ptr, len);
	    break;
	case read_short_dwvw_read_s:
	    count = dwvw_read_s(psf, ptr, len);
	    break;
	case read_short_g72x_read_s:
	    count = g72x_read_s(psf, ptr, len);
	    break;
	case read_short_gsm610_read_s:
	    count = gsm610_read_s(psf, ptr, len);
	    break;
	case read_short_host_read_d2s:
	    count = host_read_d2s(psf, ptr, len);
	    break;
	case read_short_host_read_f2s:
	    count = host_read_f2s(psf, ptr, len);
	    break;
	case read_short_ima_read_s:
	    count = ima_read_s(psf, ptr, len);
	    break;
	case read_short_interleave_read_short:
	    count = interleave_read_short(psf, ptr, len);
	    break;
	case read_short_msadpcm_read_s:
	    count = msadpcm_read_s(psf, ptr, len);
	    break;
	case read_short_nms_adpcm_read_s:
	    count = nms_adpcm_read_s(psf, ptr, len);
	    break;
	case read_short_paf24_read_s:
	    count = paf24_read_s(psf, ptr, len);
	    break;
	case read_short_pcm_read_bei2s:
	    count = pcm_read_bei2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_bes2s:
	    count = pcm_read_bes2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_bet2s:
	    count = pcm_read_bet2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_lei2s:
	    count = pcm_read_lei2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_les2s:
	    count = pcm_read_les2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_let2s:
	    count = pcm_read_let2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_sc2s:
	    count = pcm_read_sc2s(psf, ptr, len);
	    break;
	case read_short_pcm_read_uc2s:
	    count = pcm_read_uc2s(psf, ptr, len);
	    break;
	case read_short_replace_read_d2s:
	    count = replace_read_d2s(psf, ptr, len);
	    break;
	case read_short_replace_read_f2s:
	    count = replace_read_f2s(psf, ptr, len);
	    break;
	case read_short_sds_read_s:
	    count = sds_read_s(psf, ptr, len);
	    break;
	case read_short_ulaw_read_ulaw2s:
	    count = ulaw_read_ulaw2s(psf, ptr, len);
	    break;
	case read_short_vox_read_s:
	    count = vox_read_s(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_short_signature) {
	case read_short_alac_read_s:
	    count = alac_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_alaw_read_alaw2s:
	    count = alaw_read_alaw2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_dither_read_short:
	    count = dither_read_short(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_dpcm_read_dles2s:
	    count = dpcm_read_dles2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_dpcm_read_dsc2s:
	    count = dpcm_read_dsc2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_dwvw_read_s:
	    count = dwvw_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_g72x_read_s:
	    count = g72x_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_gsm610_read_s:
	    count = gsm610_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_host_read_d2s:
	    count = host_read_d2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_host_read_f2s:
	    count = host_read_f2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_ima_read_s:
	    count = ima_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_interleave_read_short:
	    count = interleave_read_short(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_msadpcm_read_s:
	    count = msadpcm_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_nms_adpcm_read_s:
	    count = nms_adpcm_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_paf24_read_s:
	    count = paf24_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_bei2s:
	    count = pcm_read_bei2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_bes2s:
	    count = pcm_read_bes2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_bet2s:
	    count = pcm_read_bet2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_lei2s:
	    count = pcm_read_lei2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_les2s:
	    count = pcm_read_les2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_let2s:
	    count = pcm_read_let2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_sc2s:
	    count = pcm_read_sc2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_pcm_read_uc2s:
	    count = pcm_read_uc2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_replace_read_d2s:
	    count = replace_read_d2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_replace_read_f2s:
	    count = replace_read_f2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_sds_read_s:
	    count = sds_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_ulaw_read_ulaw2s:
	    count = ulaw_read_ulaw2s(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_short_vox_read_s:
	    count = vox_read_s(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_int_signature) {
	case read_int_alac_read_i:
	    count = alac_read_i(psf, ptr, len);
	    break;
	case read_int_alaw_read_alaw2i:
	    count = alaw_read_alaw2i(psf, ptr, len);
	    break;
	case read_int_dither_read_int:
	    count = dither_read_int(psf, ptr, len);
	    break;
	case read_int_dpcm_read_dles2i:
	    count = dpcm_read_dles2i(psf, ptr, len);
	    break;
	case read_int_dpcm_read_dsc2i:
	    count = dpcm_read_dsc2i(psf, ptr, len);
	    break;
	case read_int_dwvw_read_i:
	    count = dwvw_read_i(psf, ptr, len);
	    break;
	case read_int_g72x_read_i:
	    count = g72x_read_i(psf, ptr, len);
	    break;
	case read_int_gsm610_read_i:
	    count = gsm610_read_i(psf, ptr, len);
	    break;
	case read_int_host_read_d2i:
	    count = host_read_d2i(psf, ptr, len);
	    break;
	case read_int_host_read_f2i:
	    count = host_read_f2i(psf, ptr, len);
	    break;
	case read_int_ima_read_i:
	    count = ima_read_i(psf, ptr, len);
	    break;
	case read_int_interleave_read_int:
	    count = interleave_read_int(psf, ptr, len);
	    break;
	case read_int_msadpcm_read_i:
	    count = msadpcm_read_i(psf, ptr, len);
	    break;
	case read_int_nms_adpcm_read_i:
	    count = nms_adpcm_read_i(psf, ptr, len);
	    break;
	case read_int_paf24_read_i:
	    count = paf24_read_i(psf, ptr, len);
	    break;
	case read_int_pcm_read_bei2i:
	    count = pcm_read_bei2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_bes2i:
	    count = pcm_read_bes2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_bet2i:
	    count = pcm_read_bet2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_lei2i:
	    count = pcm_read_lei2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_les2i:
	    count = pcm_read_les2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_let2i:
	    count = pcm_read_let2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_sc2i:
	    count = pcm_read_sc2i(psf, ptr, len);
	    break;
	case read_int_pcm_read_uc2i:
	    count = pcm_read_uc2i(psf, ptr, len);
	    break;
	case read_int_replace_read_d2i:
	    count = replace_read_d2i(psf, ptr, len);
	    break;
	case read_int_replace_read_f2i:
	    count = replace_read_f2i(psf, ptr, len);
	    break;
	case read_int_sds_read_i:
	    count = sds_read_i(psf, ptr, len);
	    break;
	case read_int_ulaw_read_ulaw2i:
	    count = ulaw_read_ulaw2i(psf, ptr, len);
	    break;
	case read_int_vox_read_i:
	    count = vox_read_i(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_int_signature) {
	case read_int_alac_read_i:
	    count = alac_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_alaw_read_alaw2i:
	    count = alaw_read_alaw2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_dither_read_int:
	    count = dither_read_int(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_dpcm_read_dles2i:
	    count = dpcm_read_dles2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_dpcm_read_dsc2i:
	    count = dpcm_read_dsc2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_dwvw_read_i:
	    count = dwvw_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_g72x_read_i:
	    count = g72x_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_gsm610_read_i:
	    count = gsm610_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_host_read_d2i:
	    count = host_read_d2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_host_read_f2i:
	    count = host_read_f2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_ima_read_i:
	    count = ima_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_interleave_read_int:
	    count = interleave_read_int(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_msadpcm_read_i:
	    count = msadpcm_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_nms_adpcm_read_i:
	    count = nms_adpcm_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_paf24_read_i:
	    count = paf24_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_bei2i:
	    count = pcm_read_bei2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_bes2i:
	    count = pcm_read_bes2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_bet2i:
	    count = pcm_read_bet2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_lei2i:
	    count = pcm_read_lei2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_les2i:
	    count = pcm_read_les2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_let2i:
	    count = pcm_read_let2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_sc2i:
	    count = pcm_read_sc2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_pcm_read_uc2i:
	    count = pcm_read_uc2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_replace_read_d2i:
	    count = replace_read_d2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_replace_read_f2i:
	    count = replace_read_f2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_sds_read_i:
	    count = sds_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_ulaw_read_ulaw2i:
	    count = ulaw_read_ulaw2i(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_int_vox_read_i:
	    count = vox_read_i(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_float_signature) {
	case read_float_alac_read_f:
	    count = alac_read_f(psf, ptr, len);
	    break;
	case read_float_alaw_read_alaw2f:
	    count = alaw_read_alaw2f(psf, ptr, len);
	    break;
	case read_float_dpcm_read_dles2f:
	    count = dpcm_read_dles2f(psf, ptr, len);
	    break;
	case read_float_dpcm_read_dsc2f:
	    count = dpcm_read_dsc2f(psf, ptr, len);
	    break;
	case read_float_dwvw_read_f:
	    count = dwvw_read_f(psf, ptr, len);
	    break;
	case read_float_g72x_read_f:
	    count = g72x_read_f(psf, ptr, len);
	    break;
	case read_float_gsm610_read_f:
	    count = gsm610_read_f(psf, ptr, len);
	    break;
	case read_float_host_read_d2f:
	    count = host_read_d2f(psf, ptr, len);
	    break;
	case read_float_host_read_f:
	    count = host_read_f(psf, ptr, len);
	    break;
	case read_float_ima_read_f:
	    count = ima_read_f(psf, ptr, len);
	    break;
	case read_float_interleave_read_float:
	    count = interleave_read_float(psf, ptr, len);
	    break;
	case read_float_msadpcm_read_f:
	    count = msadpcm_read_f(psf, ptr, len);
	    break;
	case read_float_nms_adpcm_read_f:
	    count = nms_adpcm_read_f(psf, ptr, len);
	    break;
	case read_float_paf24_read_f:
	    count = paf24_read_f(psf, ptr, len);
	    break;
	case read_float_pcm_read_bei2f:
	    count = pcm_read_bei2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_bes2f:
	    count = pcm_read_bes2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_bet2f:
	    count = pcm_read_bet2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_lei2f:
	    count = pcm_read_lei2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_les2f:
	    count = pcm_read_les2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_let2f:
	    count = pcm_read_let2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_sc2f:
	    count = pcm_read_sc2f(psf, ptr, len);
	    break;
	case read_float_pcm_read_uc2f:
	    count = pcm_read_uc2f(psf, ptr, len);
	    break;
	case read_float_replace_read_d2f:
	    count = replace_read_d2f(psf, ptr, len);
	    break;
	case read_float_replace_read_f:
	    count = replace_read_f(psf, ptr, len);
	    break;
	case read_float_sds_read_f:
	    count = sds_read_f(psf, ptr, len);
	    break;
	case read_float_ulaw_read_ulaw2f:
	    count = ulaw_read_ulaw2f(psf, ptr, len);
	    break;
	case read_float_vox_read_f:
	    count = vox_read_f(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_float_signature) {
	case read_float_alac_read_f:
	    count = alac_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_alaw_read_alaw2f:
	    count = alaw_read_alaw2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_dpcm_read_dles2f:
	    count = dpcm_read_dles2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_dpcm_read_dsc2f:
	    count = dpcm_read_dsc2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_dwvw_read_f:
	    count = dwvw_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_g72x_read_f:
	    count = g72x_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_gsm610_read_f:
	    count = gsm610_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_host_read_d2f:
	    count = host_read_d2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_host_read_f:
	    count = host_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_ima_read_f:
	    count = ima_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_interleave_read_float:
	    count = interleave_read_float(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_msadpcm_read_f:
	    count = msadpcm_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_nms_adpcm_read_f:
	    count = nms_adpcm_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_paf24_read_f:
	    count = paf24_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_bei2f:
	    count = pcm_read_bei2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_bes2f:
	    count = pcm_read_bes2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_bet2f:
	    count = pcm_read_bet2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_lei2f:
	    count = pcm_read_lei2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_les2f:
	    count = pcm_read_les2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_let2f:
	    count = pcm_read_let2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_sc2f:
	    count = pcm_read_sc2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_pcm_read_uc2f:
	    count = pcm_read_uc2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_replace_read_d2f:
	    count = replace_read_d2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_replace_read_f:
	    count = replace_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_sds_read_f:
	    count = sds_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_ulaw_read_ulaw2f:
	    count = ulaw_read_ulaw2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_float_vox_read_f:
	    count = vox_read_f(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_double_signature) {
	case read_double_alac_read_d:
	    count = alac_read_d(psf, ptr, len);
	    break;
	case read_double_alaw_read_alaw2d:
	    count = alaw_read_alaw2d(psf, ptr, len);
	    break;
	case read_double_dpcm_read_dles2d:
	    count = dpcm_read_dles2d(psf, ptr, len);
	    break;
	case read_double_dpcm_read_dsc2d:
	    count = dpcm_read_dsc2d(psf, ptr, len);
	    break;
	case read_double_dwvw_read_d:
	    count = dwvw_read_d(psf, ptr, len);
	    break;
	case read_double_g72x_read_d:
	    count = g72x_read_d(psf, ptr, len);
	    break;
	case read_double_gsm610_read_d:
	    count = gsm610_read_d(psf, ptr, len);
	    break;
	case read_double_host_read_d:
	    count = host_read_d(psf, ptr, len);
	    break;
	case read_double_host_read_f2d:
	    count = host_read_f2d(psf, ptr, len);
	    break;
	case read_double_ima_read_d:
	    count = ima_read_d(psf, ptr, len);
	    break;
	case read_double_interleave_read_double:
	    count = interleave_read_double(psf, ptr, len);
	    break;
	case read_double_msadpcm_read_d:
	    count = msadpcm_read_d(psf, ptr, len);
	    break;
	case read_double_nms_adpcm_read_d:
	    count = nms_adpcm_read_d(psf, ptr, len);
	    break;
	case read_double_paf24_read_d:
	    count = paf24_read_d(psf, ptr, len);
	    break;
	case read_double_pcm_read_bei2d:
	    count = pcm_read_bei2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_bes2d:
	    count = pcm_read_bes2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_bet2d:
	    count = pcm_read_bet2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_lei2d:
	    count = pcm_read_lei2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_les2d:
	    count = pcm_read_les2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_let2d:
	    count = pcm_read_let2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_sc2d:
	    count = pcm_read_sc2d(psf, ptr, len);
	    break;
	case read_double_pcm_read_uc2d:
	    count = pcm_read_uc2d(psf, ptr, len);
	    break;
	case read_double_replace_read_d:
	    count = replace_read_d(psf, ptr, len);
	    break;
	case read_double_replace_read_f2d:
	    count = replace_read_f2d(psf, ptr, len);
	    break;
	case read_double_sds_read_d:
	    count = sds_read_d(psf, ptr, len);
	    break;
	case read_double_ulaw_read_ulaw2d:
	    count = ulaw_read_ulaw2d(psf, ptr, len);
	    break;
	case read_double_vox_read_d:
	    count = vox_read_d(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->read_double_signature) {
	case read_double_alac_read_d:
	    count = alac_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_alaw_read_alaw2d:
	    count = alaw_read_alaw2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_dpcm_read_dles2d:
	    count = dpcm_read_dles2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_dpcm_read_dsc2d:
	    count = dpcm_read_dsc2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_dwvw_read_d:
	    count = dwvw_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_g72x_read_d:
	    count = g72x_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_gsm610_read_d:
	    count = gsm610_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_host_read_d:
	    count = host_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_host_read_f2d:
	    count = host_read_f2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_ima_read_d:
	    count = ima_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_interleave_read_double:
	    count = interleave_read_double(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_msadpcm_read_d:
	    count = msadpcm_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_nms_adpcm_read_d:
	    count = nms_adpcm_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_paf24_read_d:
	    count = paf24_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_bei2d:
	    count = pcm_read_bei2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_bes2d:
	    count = pcm_read_bes2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_bet2d:
	    count = pcm_read_bet2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_lei2d:
	    count = pcm_read_lei2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_les2d:
	    count = pcm_read_les2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_let2d:
	    count = pcm_read_let2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_sc2d:
	    count = pcm_read_sc2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_pcm_read_uc2d:
	    count = pcm_read_uc2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_replace_read_d:
	    count = replace_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_replace_read_f2d:
	    count = replace_read_f2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_sds_read_d:
	    count = sds_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_ulaw_read_ulaw2d:
	    count = ulaw_read_ulaw2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case read_double_vox_read_d:
	    count = vox_read_d(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	if (psf->last_op != SFM_WRITE) {
		...
		switch (psf->seek_signature) {
		case seek_aiff_ima_seek:
		    tmp = aiff_ima_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_alac_seek:
		    tmp = alac_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_dpcm_seek:
		    tmp = dpcm_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_dwvw_seek:
		    tmp = dwvw_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_g72x_seek:
		    tmp = g72x_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_gsm610_seek:
		    tmp = gsm610_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_interleave_seek:
		    tmp = interleave_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_msadpcm_seek:
		    tmp = msadpcm_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_nms_adpcm_seek:
		    tmp = nms_adpcm_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_paf24_seek:
		    tmp = paf24_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_psf_default_seek:
		    tmp = psf_default_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_sds_seek:
		    tmp = sds_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		case seek_wavlike_ima_seek:
		    tmp = wavlike_ima_seek(psf, SFM_WRITE, psf->write_current);
		    break;
		default:
		    invalidFunctionWasCalled();
		    break;
	...
	{	
		switch (psf->write_header_signature) {
		case write_header_aiff_write_header:
		    psf->error = aiff_write_header(psf, SF_FALSE);
		    break;
		case write_header_au_write_header:
		    psf->error = au_write_header(psf, SF_FALSE);
		    break;
		case write_header_avr_write_header:
		    psf->error = avr_write_header(psf, SF_FALSE);
		    break;
		case write_header_caf_write_header:
		    psf->error = caf_write_header(psf, SF_FALSE);
		    break;
		case write_header_htk_write_header:
		    psf->error = htk_write_header(psf, SF_FALSE);
		    break;
		case write_header_ircam_write_header:
		    psf->error = ircam_write_header(psf, SF_FALSE);
		    break;
		case write_header_mat4_write_header:
		    psf->error = mat4_write_header(psf, SF_FALSE);
		    break;
		case write_header_mat5_write_header:
		    psf->error = mat5_write_header(psf, SF_FALSE);
		    break;
		case write_header_mpc2k_write_header:
		    psf->error = mpc2k_write_header(psf, SF_FALSE);
		    break;
		case write_header_nist_write_header:
		    psf->error = nist_write_header(psf, SF_FALSE);
		    break;
		case write_header_paf_write_header:
		    psf->error = paf_write_header(psf, SF_FALSE);
		    break;
		case write_header_pvf_write_header:
		    psf->error = pvf_write_header(psf, SF_FALSE);
		    break;
		case write_header_rf64_write_header:
		    psf->error = rf64_write_header(psf, SF_FALSE);
		    break;
		case write_header_sds_write_header:
		    psf->error = sds_write_header(psf, SF_FALSE);
		    break;
		case write_header_svx_write_header:
		    psf->error = svx_write_header(psf, SF_FALSE);
		    break;
		case write_header_voc_write_header:
		    psf->error = voc_write_header(psf, SF_FALSE);
		    break;
		case write_header_w64_write_header:
		    psf->error = w64_write_header(psf, SF_FALSE);
		    break;
		case write_header_wav_write_header:
		    psf->error = wav_write_header(psf, SF_FALSE);
		    break;
		case write_header_wve_write_header:
		    psf->error = wve_write_header(psf, SF_FALSE);
		    break;
		case write_header_xi_write_header:
		    psf->error = xi_write_header(psf, SF_FALSE);
		    break;
		default:
		    invalidFunctionWasCalled();
		    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_short_signature) {
	case write_short_alac_write_s:
	    count = alac_write_s(psf, ptr, len);
	    break;
	case write_short_alaw_write_s2alaw:
	    count = alaw_write_s2alaw(psf, ptr, len);
	    break;
	case write_short_dither_write_short:
	    count = dither_write_short(psf, ptr, len);
	    break;
	case write_short_dpcm_write_s2dles:
	    count = dpcm_write_s2dles(psf, ptr, len);
	    break;
	case write_short_dpcm_write_s2dsc:
	    count = dpcm_write_s2dsc(psf, ptr, len);
	    break;
	case write_short_dwvw_write_s:
	    count = dwvw_write_s(psf, ptr, len);
	    break;
	case write_short_g72x_write_s:
	    count = g72x_write_s(psf, ptr, len);
	    break;
	case write_short_gsm610_write_s:
	    count = gsm610_write_s(psf, ptr, len);
	    break;
	case write_short_host_write_s2d:
	    count = host_write_s2d(psf, ptr, len);
	    break;
	case write_short_host_write_s2f:
	    count = host_write_s2f(psf, ptr, len);
	    break;
	case write_short_ima_write_s:
	    count = ima_write_s(psf, ptr, len);
	    break;
	case write_short_msadpcm_write_s:
	    count = msadpcm_write_s(psf, ptr, len);
	    break;
	case write_short_nms_adpcm_write_s:
	    count = nms_adpcm_write_s(psf, ptr, len);
	    break;
	case write_short_paf24_write_s:
	    count = paf24_write_s(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2bei:
	    count = pcm_write_s2bei(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2bes:
	    count = pcm_write_s2bes(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2bet:
	    count = pcm_write_s2bet(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2lei:
	    count = pcm_write_s2lei(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2les:
	    count = pcm_write_s2les(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2let:
	    count = pcm_write_s2let(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2sc:
	    count = pcm_write_s2sc(psf, ptr, len);
	    break;
	case write_short_pcm_write_s2uc:
	    count = pcm_write_s2uc(psf, ptr, len);
	    break;
	case write_short_replace_write_s2d:
	    count = replace_write_s2d(psf, ptr, len);
	    break;
	case write_short_replace_write_s2f:
	    count = replace_write_s2f(psf, ptr, len);
	    break;
	case write_short_sds_write_s:
	    count = sds_write_s(psf, ptr, len);
	    break;
	case write_short_ulaw_write_s2ulaw:
	    count = ulaw_write_s2ulaw(psf, ptr, len);
	    break;
	case write_short_vox_write_s:
	    count = vox_write_s(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_short_signature) {
	case write_short_alac_write_s:
	    count = alac_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_alaw_write_s2alaw:
	    count = alaw_write_s2alaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_dither_write_short:
	    count = dither_write_short(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_dpcm_write_s2dles:
	    count = dpcm_write_s2dles(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_dpcm_write_s2dsc:
	    count = dpcm_write_s2dsc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_dwvw_write_s:
	    count = dwvw_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_g72x_write_s:
	    count = g72x_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_gsm610_write_s:
	    count = gsm610_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_host_write_s2d:
	    count = host_write_s2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_host_write_s2f:
	    count = host_write_s2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_ima_write_s:
	    count = ima_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_msadpcm_write_s:
	    count = msadpcm_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_nms_adpcm_write_s:
	    count = nms_adpcm_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_paf24_write_s:
	    count = paf24_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2bei:
	    count = pcm_write_s2bei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2bes:
	    count = pcm_write_s2bes(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2bet:
	    count = pcm_write_s2bet(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2lei:
	    count = pcm_write_s2lei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2les:
	    count = pcm_write_s2les(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2let:
	    count = pcm_write_s2let(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2sc:
	    count = pcm_write_s2sc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_pcm_write_s2uc:
	    count = pcm_write_s2uc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_replace_write_s2d:
	    count = replace_write_s2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_replace_write_s2f:
	    count = replace_write_s2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_sds_write_s:
	    count = sds_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_ulaw_write_s2ulaw:
	    count = ulaw_write_s2ulaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_short_vox_write_s:
	    count = vox_write_s(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_int_signature) {
	case write_int_alac_write_i:
	    count = alac_write_i(psf, ptr, len);
	    break;
	case write_int_alaw_write_i2alaw:
	    count = alaw_write_i2alaw(psf, ptr, len);
	    break;
	case write_int_dither_write_int:
	    count = dither_write_int(psf, ptr, len);
	    break;
	case write_int_dpcm_write_i2dles:
	    count = dpcm_write_i2dles(psf, ptr, len);
	    break;
	case write_int_dpcm_write_i2dsc:
	    count = dpcm_write_i2dsc(psf, ptr, len);
	    break;
	case write_int_dwvw_write_i:
	    count = dwvw_write_i(psf, ptr, len);
	    break;
	case write_int_g72x_write_i:
	    count = g72x_write_i(psf, ptr, len);
	    break;
	case write_int_gsm610_write_i:
	    count = gsm610_write_i(psf, ptr, len);
	    break;
	case write_int_host_write_i2d:
	    count = host_write_i2d(psf, ptr, len);
	    break;
	case write_int_host_write_i2f:
	    count = host_write_i2f(psf, ptr, len);
	    break;
	case write_int_ima_write_i:
	    count = ima_write_i(psf, ptr, len);
	    break;
	case write_int_msadpcm_write_i:
	    count = msadpcm_write_i(psf, ptr, len);
	    break;
	case write_int_nms_adpcm_write_i:
	    count = nms_adpcm_write_i(psf, ptr, len);
	    break;
	case write_int_paf24_write_i:
	    count = paf24_write_i(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2bei:
	    count = pcm_write_i2bei(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2bes:
	    count = pcm_write_i2bes(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2bet:
	    count = pcm_write_i2bet(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2lei:
	    count = pcm_write_i2lei(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2les:
	    count = pcm_write_i2les(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2let:
	    count = pcm_write_i2let(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2sc:
	    count = pcm_write_i2sc(psf, ptr, len);
	    break;
	case write_int_pcm_write_i2uc:
	    count = pcm_write_i2uc(psf, ptr, len);
	    break;
	case write_int_replace_write_i2d:
	    count = replace_write_i2d(psf, ptr, len);
	    break;
	case write_int_replace_write_i2f:
	    count = replace_write_i2f(psf, ptr, len);
	    break;
	case write_int_sds_write_i:
	    count = sds_write_i(psf, ptr, len);
	    break;
	case write_int_ulaw_write_i2ulaw:
	    count = ulaw_write_i2ulaw(psf, ptr, len);
	    break;
	case write_int_vox_write_i:
	    count = vox_write_i(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_int_signature) {
	case write_int_alac_write_i:
	    count = alac_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_alaw_write_i2alaw:
	    count = alaw_write_i2alaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_dither_write_int:
	    count = dither_write_int(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_dpcm_write_i2dles:
	    count = dpcm_write_i2dles(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_dpcm_write_i2dsc:
	    count = dpcm_write_i2dsc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_dwvw_write_i:
	    count = dwvw_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_g72x_write_i:
	    count = g72x_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_gsm610_write_i:
	    count = gsm610_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_host_write_i2d:
	    count = host_write_i2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_host_write_i2f:
	    count = host_write_i2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_ima_write_i:
	    count = ima_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_msadpcm_write_i:
	    count = msadpcm_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_nms_adpcm_write_i:
	    count = nms_adpcm_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_paf24_write_i:
	    count = paf24_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2bei:
	    count = pcm_write_i2bei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2bes:
	    count = pcm_write_i2bes(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2bet:
	    count = pcm_write_i2bet(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2lei:
	    count = pcm_write_i2lei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2les:
	    count = pcm_write_i2les(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2let:
	    count = pcm_write_i2let(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2sc:
	    count = pcm_write_i2sc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_pcm_write_i2uc:
	    count = pcm_write_i2uc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_replace_write_i2d:
	    count = replace_write_i2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_replace_write_i2f:
	    count = replace_write_i2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_sds_write_i:
	    count = sds_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_ulaw_write_i2ulaw:
	    count = ulaw_write_i2ulaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_int_vox_write_i:
	    count = vox_write_i(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_float_signature) {
	case write_float_alac_write_f:
	    count = alac_write_f(psf, ptr, len);
	    break;
	case write_float_alaw_write_f2alaw:
	    count = alaw_write_f2alaw(psf, ptr, len);
	    break;
	case write_float_dither_write_float:
	    count = dither_write_float(psf, ptr, len);
	    break;
	case write_float_dpcm_write_f2dles:
	    count = dpcm_write_f2dles(psf, ptr, len);
	    break;
	case write_float_dpcm_write_f2dsc:
	    count = dpcm_write_f2dsc(psf, ptr, len);
	    break;
	case write_float_dwvw_write_f:
	    count = dwvw_write_f(psf, ptr, len);
	    break;
	case write_float_g72x_write_f:
	    count = g72x_write_f(psf, ptr, len);
	    break;
	case write_float_gsm610_write_f:
	    count = gsm610_write_f(psf, ptr, len);
	    break;
	case write_float_host_write_f:
	    count = host_write_f(psf, ptr, len);
	    break;
	case write_float_host_write_f2d:
	    count = host_write_f2d(psf, ptr, len);
	    break;
	case write_float_ima_write_f:
	    count = ima_write_f(psf, ptr, len);
	    break;
	case write_float_msadpcm_write_f:
	    count = msadpcm_write_f(psf, ptr, len);
	    break;
	case write_float_nms_adpcm_write_f:
	    count = nms_adpcm_write_f(psf, ptr, len);
	    break;
	case write_float_paf24_write_f:
	    count = paf24_write_f(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2bei:
	    count = pcm_write_f2bei(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2bes:
	    count = pcm_write_f2bes(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2bet:
	    count = pcm_write_f2bet(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2lei:
	    count = pcm_write_f2lei(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2les:
	    count = pcm_write_f2les(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2let:
	    count = pcm_write_f2let(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2sc:
	    count = pcm_write_f2sc(psf, ptr, len);
	    break;
	case write_float_pcm_write_f2uc:
	    count = pcm_write_f2uc(psf, ptr, len);
	    break;
	case write_float_replace_write_f:
	    count = replace_write_f(psf, ptr, len);
	    break;
	case write_float_replace_write_f2d:
	    count = replace_write_f2d(psf, ptr, len);
	    break;
	case write_float_sds_write_f:
	    count = sds_write_f(psf, ptr, len);
	    break;
	case write_float_ulaw_write_f2ulaw:
	    count = ulaw_write_f2ulaw(psf, ptr, len);
	    break;
	case write_float_vox_write_f:
	    count = vox_write_f(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_float_signature) {
	case write_float_alac_write_f:
	    count = alac_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_alaw_write_f2alaw:
	    count = alaw_write_f2alaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_dither_write_float:
	    count = dither_write_float(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_dpcm_write_f2dles:
	    count = dpcm_write_f2dles(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_dpcm_write_f2dsc:
	    count = dpcm_write_f2dsc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_dwvw_write_f:
	    count = dwvw_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_g72x_write_f:
	    count = g72x_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_gsm610_write_f:
	    count = gsm610_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_host_write_f:
	    count = host_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_host_write_f2d:
	    count = host_write_f2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_ima_write_f:
	    count = ima_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_msadpcm_write_f:
	    count = msadpcm_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_nms_adpcm_write_f:
	    count = nms_adpcm_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_paf24_write_f:
	    count = paf24_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2bei:
	    count = pcm_write_f2bei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2bes:
	    count = pcm_write_f2bes(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2bet:
	    count = pcm_write_f2bet(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2lei:
	    count = pcm_write_f2lei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2les:
	    count = pcm_write_f2les(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2let:
	    count = pcm_write_f2let(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2sc:
	    count = pcm_write_f2sc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_pcm_write_f2uc:
	    count = pcm_write_f2uc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_replace_write_f:
	    count = replace_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_replace_write_f2d:
	    count = replace_write_f2d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_sds_write_f:
	    count = sds_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_ulaw_write_f2ulaw:
	    count = ulaw_write_f2ulaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_float_vox_write_f:
	    count = vox_write_f(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_double_signature) {
	case write_double_alac_write_d:
	    count = alac_write_d(psf, ptr, len);
	    break;
	case write_double_alaw_write_d2alaw:
	    count = alaw_write_d2alaw(psf, ptr, len);
	    break;
	case write_double_dither_write_double:
	    count = dither_write_double(psf, ptr, len);
	    break;
	case write_double_dpcm_write_d2dles:
	    count = dpcm_write_d2dles(psf, ptr, len);
	    break;
	case write_double_dpcm_write_d2dsc:
	    count = dpcm_write_d2dsc(psf, ptr, len);
	    break;
	case write_double_dwvw_write_d:
	    count = dwvw_write_d(psf, ptr, len);
	    break;
	case write_double_g72x_write_d:
	    count = g72x_write_d(psf, ptr, len);
	    break;
	case write_double_gsm610_write_d:
	    count = gsm610_write_d(psf, ptr, len);
	    break;
	case write_double_host_write_d:
	    count = host_write_d(psf, ptr, len);
	    break;
	case write_double_host_write_d2f:
	    count = host_write_d2f(psf, ptr, len);
	    break;
	case write_double_ima_write_d:
	    count = ima_write_d(psf, ptr, len);
	    break;
	case write_double_msadpcm_write_d:
	    count = msadpcm_write_d(psf, ptr, len);
	    break;
	case write_double_nms_adpcm_write_d:
	    count = nms_adpcm_write_d(psf, ptr, len);
	    break;
	case write_double_paf24_write_d:
	    count = paf24_write_d(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2bei:
	    count = pcm_write_d2bei(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2bes:
	    count = pcm_write_d2bes(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2bet:
	    count = pcm_write_d2bet(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2lei:
	    count = pcm_write_d2lei(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2les:
	    count = pcm_write_d2les(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2let:
	    count = pcm_write_d2let(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2sc:
	    count = pcm_write_d2sc(psf, ptr, len);
	    break;
	case write_double_pcm_write_d2uc:
	    count = pcm_write_d2uc(psf, ptr, len);
	    break;
	case write_double_replace_write_d:
	    count = replace_write_d(psf, ptr, len);
	    break;
	case write_double_replace_write_d2f:
	    count = replace_write_d2f(psf, ptr, len);
	    break;
	case write_double_sds_write_d:
	    count = sds_write_d(psf, ptr, len);
	    break;
	case write_double_ulaw_write_d2ulaw:
	    count = ulaw_write_d2ulaw(psf, ptr, len);
	    break;
	case write_double_vox_write_d:
	    count = vox_write_d(psf, ptr, len);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	SF_PRIVATE 	*psf ;
	...
	switch (psf->write_double_signature) {
	case write_double_alac_write_d:
	    count = alac_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_alaw_write_d2alaw:
	    count = alaw_write_d2alaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_dither_write_double:
	    count = dither_write_double(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_dpcm_write_d2dles:
	    count = dpcm_write_d2dles(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_dpcm_write_d2dsc:
	    count = dpcm_write_d2dsc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_dwvw_write_d:
	    count = dwvw_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_g72x_write_d:
	    count = g72x_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_gsm610_write_d:
	    count = gsm610_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_host_write_d:
	    count = host_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_host_write_d2f:
	    count = host_write_d2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_ima_write_d:
	    count = ima_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_msadpcm_write_d:
	    count = msadpcm_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_nms_adpcm_write_d:
	    count = nms_adpcm_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_paf24_write_d:
	    count = paf24_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2bei:
	    count = pcm_write_d2bei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2bes:
	    count = pcm_write_d2bes(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2bet:
	    count = pcm_write_d2bet(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2lei:
	    count = pcm_write_d2lei(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2les:
	    count = pcm_write_d2les(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2let:
	    count = pcm_write_d2let(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2sc:
	    count = pcm_write_d2sc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_pcm_write_d2uc:
	    count = pcm_write_d2uc(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_replace_write_d:
	    count = replace_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_replace_write_d2f:
	    count = replace_write_d2f(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_sds_write_d:
	    count = sds_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_ulaw_write_d2ulaw:
	    count = ulaw_write_d2ulaw(psf, ptr, frames * psf->sf.channels);
	    break;
	case write_double_vox_write_d:
	    count = vox_write_d(psf, ptr, frames * psf->sf.channels);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
...
{	uint32_t k ;
	...
	if (psf->codec_close)
	{	switch (psf->codec_close_signature) {
	case codec_close_alac_close:
	    error = alac_close(psf);
	    break;
	case codec_close_dwvw_close:
	    error = dwvw_close(psf);
	    break;
	case codec_close_g72x_close:
	    error = g72x_close(psf);
	    break;
	case codec_close_gsm610_close:
	    error = gsm610_close(psf);
	    break;
	case codec_close_ima_close:
	    error = ima_close(psf);
	    break;
	case codec_close_msadpcm_close:
	    error = msadpcm_close(psf);
	    break;
	case codec_close_nms_adpcm_close:
	    error = nms_adpcm_close(psf);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
	}
		/* To prevent it being called in switch (psf->container_close_signature) {
...
{	int		error, format ;
	...
	psf->seek_signature = seek_psf_default_seek;
...
{	SF_PRIVATE 	*psf ;
	...
	if (psf->set_chunk)
		switch (psf->set_chunk_signature) {
		case set_chunk_aiff_set_chunk:
		    return aiff_set_chunk(psf, chunk_info);
		case set_chunk_caf_set_chunk:
		    return caf_set_chunk(psf, chunk_info);
		case set_chunk_rf64_set_chunk:
		    return rf64_set_chunk(psf, chunk_info);
		case set_chunk_wav_set_chunk:
		    return wav_set_chunk(psf, chunk_info);
		default:
		    invalidFunctionWasCalled();
		}
...
{	SF_PRIVATE 	*psf ;
	...
	if (psf->next_chunk_iterator)
		switch (psf->next_chunk_iterator_signature) {
		case next_chunk_iterator_aiff_next_chunk_iterator:
		    return aiff_next_chunk_iterator(psf, iterator);
		case next_chunk_iterator_caf_next_chunk_iterator:
		    return caf_next_chunk_iterator(psf, iterator);
		case next_chunk_iterator_rf64_next_chunk_iterator:
		    return rf64_next_chunk_iterator(psf, iterator);
		case next_chunk_iterator_wav_next_chunk_iterator:
		    return wav_next_chunk_iterator(psf, iterator);
		default:
		    invalidFunctionWasCalled();
		}
...
{	SF_PRIVATE 	*psf ;
	...
	if (psf->get_chunk_size)
		switch (psf->get_chunk_size_signature) {
		case get_chunk_size_aiff_get_chunk_size:
		    return aiff_get_chunk_size(psf, iterator, chunk_info);
		case get_chunk_size_caf_get_chunk_size:
		    return caf_get_chunk_size(psf, iterator, chunk_info);
		case get_chunk_size_rf64_get_chunk_size:
		    return rf64_get_chunk_size(psf, iterator, chunk_info);
		case get_chunk_size_wav_get_chunk_size:
		    return wav_get_chunk_size(psf, iterator, chunk_info);
		default:
		    invalidFunctionWasCalled();
		}
...
{	SF_PRIVATE	*psf ;
	...
	if (psf->get_chunk_data)
		switch (psf->get_chunk_data_signature) {
		case get_chunk_data_aiff_get_chunk_data:
		    return aiff_get_chunk_data(psf, iterator, chunk_info);
		case get_chunk_data_caf_get_chunk_data:
		    return caf_get_chunk_data(psf, iterator, chunk_info);
		case get_chunk_data_rf64_get_chunk_data:
		    return rf64_get_chunk_data(psf, iterator, chunk_info);
		case get_chunk_data_wav_get_chunk_data:
		    return wav_get_chunk_data(psf, iterator, chunk_info);
		default:
		    invalidFunctionWasCalled();
		}
