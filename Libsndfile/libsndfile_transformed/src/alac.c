...
{	int error ;
	...
	psf->codec_close_signature = codec_close_alac_close;
	...
	psf->byterate_signature = byterate_alac_byterate;
...
{	ALAC_PRIVATE *plac ;
	...
	{	ALAC_ENCODER *penc = &plac->u.encoder ;
		...
		case write_header_paf_write_header:
		    paf_write_header(psf, 1);
		    break;
		case write_header_pvf_write_header:
		    pvf_write_header(psf, 1);
		    break;
...
{	ALAC_PRIVATE	*plac ;
	...
	{	case 16 :
		...
		case 32 :
			...
			psf->read_short_signature = read_short_alac_read_s;
			psf->read_int = alac_read_i;
			psf->read_int_signature = read_int_alac_read_i;
			psf->read_float = alac_read_f;
			psf->read_float_signature = read_float_alac_read_f;
			psf->read_double = alac_read_d;
			psf->read_double_signature = read_double_alac_read_d;
			break ;
	...
	psf->codec_close_signature = codec_close_alac_close;
	psf->seek = alac_seek;
	psf->seek_signature = seek_alac_seek;
...
{	ALAC_PRIVATE	*plac ;
	...
	psf->write_short_signature = write_short_alac_write_s;
	psf->write_int = alac_write_i;
	psf->write_int_signature = write_int_alac_write_i;
	psf->write_float = alac_write_f;
	psf->write_float_signature = write_float_alac_write_f;
	psf->write_double = alac_write_d;
	psf->write_double_signature = write_double_alac_write_d;
...
{	ALAC_PRIVATE *plac ;
	...
	{	writecount = (plac->frames_per_block - plac->partial_block_frames) * plac->channels ;
		...
		if (psf->add_clipping) {
			psf_f2i_clip_array (ptr, iptr, writecount, psf->norm_float) ;
		}
		else {
			psf_f2i_array (ptr, iptr, writecount, psf->norm_float) ;
		}
...
{	ALAC_PRIVATE *plac ;
	...
	{	writecount = (plac->frames_per_block - plac->partial_block_frames) * plac->channels ;
		...
		// convert (ptr, iptr, writecount, psf->norm_float) ;
		if (psf->add_clipping) {
		psf_d2i_clip_array (ptr, iptr, writecount, psf->norm_float) ;
		}
		else {
		psf_d2i_array (ptr, iptr, writecount, psf->norm_float) ;
		}
...
{	SF_CHUNK_INFO chunk_info ;
	...
	case get_chunk_size_aiff_get_chunk_size:
	    aiff_get_chunk_size(psf, chunk_iterator, &chunk_info);
	    break;
	case get_chunk_size_caf_get_chunk_size:
	    caf_get_chunk_size(psf, chunk_iterator, &chunk_info);
	    break;
	case get_chunk_size_rf64_get_chunk_size:
	    rf64_get_chunk_size(psf, chunk_iterator, &chunk_info);
	    break;
	case get_chunk_size_wav_get_chunk_size:
	    wav_get_chunk_size(psf, chunk_iterator, &chunk_info);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
	...
	case get_chunk_data_aiff_get_chunk_data:
	    bcount = aiff_get_chunk_data(psf, chunk_iterator, &chunk_info);
	    break;
	case get_chunk_data_caf_get_chunk_data:
	    bcount = caf_get_chunk_data(psf, chunk_iterator, &chunk_info);
	    break;
	case get_chunk_data_rf64_get_chunk_data:
	    bcount = rf64_get_chunk_data(psf, chunk_iterator, &chunk_info);
	    break;
	case get_chunk_data_wav_get_chunk_data:
	    bcount = wav_get_chunk_data(psf, chunk_iterator, &chunk_info);
	    break;
	default:
	    invalidFunctionWasCalled();
	    break;
	}
	if (bcount != SF_ERR_NO_ERROR)
	{	while (chunk_iterator)
			switch (psf->next_chunk_iterator_signature) {
			case next_chunk_iterator_aiff_next_chunk_iterator:
			    chunk_iterator = aiff_next_chunk_iterator(psf, chunk_iterator);
			    break;
			case next_chunk_iterator_caf_next_chunk_iterator:
			    chunk_iterator = caf_next_chunk_iterator(psf, chunk_iterator);
			    break;
			case next_chunk_iterator_rf64_next_chunk_iterator:
			    chunk_iterator = rf64_next_chunk_iterator(psf, chunk_iterator);
			    break;
			case next_chunk_iterator_wav_next_chunk_iterator:
			    chunk_iterator = wav_next_chunk_iterator(psf, chunk_iterator);
			    break;
			default:
			    invalidFunctionWasCalled();
			    break;
