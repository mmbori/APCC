...
{	static double	orig_buffer [BUFFER_SIZE] ;
	...
	encoder_state.encoder_signature = encoder_g723_24_encoder;
	encoder_state.codec_bits = 3 ;

	private_init_state (&decoder_state) ;
	decoder_state.decoder = g723_24_decoder;
	decoder_state.decoder_signature = decoder_g723_24_decoder;
	...
	max_err = 0 ;
	for (k = 0 ; k < BUFFER_SIZE ; k++)
	{	switch (encoder_state.encoder_signature) {
	case encoder_g721_encoder:
	    code = g721_encoder(orig [k], &encoder_state);
	    break;
	case encoder_g723_16_encoder:
	    code = g723_16_encoder(orig [k], &encoder_state);
	    break;
	case encoder_g723_24_encoder:
	    code = g723_24_encoder(orig [k], &encoder_state);
	    break;
	case encoder_g723_40_encoder:
	    code = g723_40_encoder(orig [k], &encoder_state);
	    break;
	default:
