...
sf_count_t mpeg_dec_seek(SF_PRIVATE *psf, int whence, sf_count_t count);
...
{	BUF_UNION ubuf ;
	...
	{	readlen = mpeg_dec_decode (psf, ubuf.fbuf, SF_MIN (buflen, len - total)) ;
		...
		if (psf->add_clipping) {
		psf_f2s_clip_array (ubuf.fbuf, ptr + total, readlen, SF_TRUE) ;
		}
		else {
		psf_f2s_array (ubuf.fbuf, ptr + total, readlen, SF_TRUE) ;
		}
		} ;
...
{	BUF_UNION ubuf ;
	...
	{	readlen = mpeg_dec_decode (psf, ubuf.fbuf, SF_MIN (buflen, len - total)) ;
		...
		// convert (ubuf.fbuf, ptr + total, readlen, SF_TRUE) ;
		if (psf->add_clipping) {
		psf_f2i_clip_array (ubuf.fbuf, ptr + total, readlen, SF_TRUE) ;
		}
		else {
		psf_f2i_array (ubuf.fbuf, ptr + total, readlen, SF_TRUE) ;
		}
		} ;
...
{	MPEG_DEC_PRIVATE *pmp3d ;
	...
	*/
	...
	psf->codec_close_signature = codec_close_mpeg_dec_close;
	...
	*/
	...
	psf->read_short_signature = read_short_mpeg_dec_read_s;
	psf->read_int = mpeg_dec_read_i;
	psf->read_int_signature = read_int_mpeg_dec_read_i;
	psf->read_float = mpeg_dec_read_f;
	psf->read_float_signature = read_float_mpeg_dec_read_f;
	psf->read_double = mpeg_dec_read_d;
	psf->read_double_signature = read_double_mpeg_dec_read_d;
	psf->seek = mpeg_dec_seek;
	psf->seek_signature = seek_mpeg_dec_seek;
	psf->byterate = mpeg_dec_byterate;
	psf->byterate_signature = byterate_mpeg_dec_byterate;
