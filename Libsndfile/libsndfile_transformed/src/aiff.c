...
SF_CHUNK_ITERATOR *  aiff_next_chunk_iterator(SF_PRIVATE *psf,
					      SF_CHUNK_ITERATOR * iterator) ;
int aiff_get_chunk_size(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
			SF_CHUNK_INFO * chunk_info) ;
int aiff_get_chunk_data(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
			SF_CHUNK_INFO * chunk_info) ;
...
{	COMM_CHUNK comm_fmt ;
	...
	psf->container_close_signature = container_close_aiff_close;
	...
	{	if ((error = aiff_read_header (psf, &comm_fmt)))
		...
		psf->next_chunk_iterator_signature = next_chunk_iterator_aiff_next_chunk_iterator;
		psf->get_chunk_size = aiff_get_chunk_size;
		psf->get_chunk_size_signature = get_chunk_size_aiff_get_chunk_size;
		psf->get_chunk_data = aiff_get_chunk_data;
		psf->get_chunk_data_signature = get_chunk_data_aiff_get_chunk_data;
	...
	{	if (psf->is_pipe)
		...
		psf->write_header_signature = write_header_aiff_write_header;
		psf->set_chunk = aiff_set_chunk;
		psf->set_chunk_signature = set_chunk_aiff_set_chunk;
		} ;

	psf->command = aiff_command;
	psf->command_signature = command_aiff_command;
...
int aiff_command(SF_PRIVATE * psf, int command, void * UNUSED(data),
		 int UNUSED(datasize))
{	AIFF_PRIVATE	*paiff ;
