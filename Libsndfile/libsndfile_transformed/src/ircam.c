...
{	int		subformat ;
	...
	{	if ((SF_CONTAINER (psf->sf.format)) != SF_FORMAT_IRCAM)
		...
		psf->write_header_signature = write_header_ircam_write_header;
		} ;

	psf->container_close = ircam_close;
	psf->container_close_signature = container_close_ircam_close;
...
int ircam_write_header(SF_PRIVATE *psf, int UNUSED(calc_length))
{	int			encoding ;
	float		samplerate ;
	sf_count_t	current ;
