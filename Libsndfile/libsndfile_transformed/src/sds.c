...
{	int bitwidth, frames ;
	...
	int reader_signature;
	int writer_signature;
} SDS_PRIVATE ;
...
sf_count_t sds_seek(SF_PRIVATE *psf, int mode, sf_count_t offset);
...
{	SDS_PRIVATE	*psds ;
	...
	{	if (sds_write_header (psf, SF_FALSE))
		...
		psf->write_header_signature = write_header_sds_write_header;
	...
	psf->container_close_signature = container_close_sds_close;
	psf->seek = sds_seek;
	psf->seek_signature = seek_sds_seek;
	psf->byterate = sds_byterate;
	psf->byterate_signature = byterate_sds_byterate;
...
int sds_close(SF_PRIVATE *psf)
{
	...
	{	SDS_PRIVATE *psds ;
		...
		{	memset (&(psds->write_data [psds->write_count]), 0, (psds->samplesperblock - psds->write_count) * sizeof (int)) ;
			switch (psds->writer_signature) {
			case writer_sds_2byte_write:
			    sds_2byte_write(psf, psds);
			    break;
			case writer_sds_3byte_write:
			    sds_3byte_write(psf, psds);
			    break;
			case writer_sds_4byte_write:
			    sds_4byte_write(psf, psds);
			    break;
			default:
...
sds_init (SF_PRIVATE *psf, SDS_PRIVATE *psds)
{
	...
	psds->reader_signature = reader_sds_2byte_read;
		psds->writer = sds_2byte_write;
		psds->writer_signature = writer_sds_2byte_write;
		psds->samplesperblock = SDS_AUDIO_BYTES_PER_BLOCK / 2 ;
		}
	else if (psds->bitwidth < 21)
	{	psds->reader = sds_3byte_read;
	psds->reader_signature = reader_sds_3byte_read;
		psds->writer = sds_3byte_write;
		psds->writer_signature = writer_sds_3byte_write;
		psds->samplesperblock = SDS_AUDIO_BYTES_PER_BLOCK / 3 ;
		}
	else
	{	psds->reader = sds_4byte_read;
	psds->reader_signature = reader_sds_4byte_read;
		psds->writer = sds_4byte_write;
		psds->writer_signature = writer_sds_4byte_write;
		psds->samplesperblock = SDS_AUDIO_BYTES_PER_BLOCK / 4 ;
		} ;

	if (psf->file.mode == SFM_READ || psf->file.mode == SFM_RDWR)
	{	psf->read_short = sds_read_s;
	psf->read_short_signature = read_short_sds_read_s;
		psf->read_int = sds_read_i;
		psf->read_int_signature = read_int_sds_read_i;
		psf->read_float = sds_read_f;
		psf->read_float_signature = read_float_sds_read_f;
		psf->read_double = sds_read_d;
		psf->read_double_signature = read_double_sds_read_d;

		/* Read first block. */
		switch (psds->reader_signature) {
		case reader_sds_2byte_read:
		    sds_2byte_read(psf, psds);
		    break;
		case reader_sds_3byte_read:
		    sds_3byte_read(psf, psds);
		    break;
		case reader_sds_4byte_read:
		    sds_4byte_read(psf, psds);
		    break;
		default:
		    invalidFunctionWasCalled();
		    break;
	...
	psf->write_short_signature = write_short_sds_write_s;
		psf->write_int = sds_write_i;
		psf->write_int_signature = write_int_sds_write_i;
		psf->write_float = sds_write_f;
		psf->write_float_signature = write_float_sds_write_f;
		psf->write_double = sds_write_d;
		psf->write_double_signature = write_double_sds_write_d;
		} ;
