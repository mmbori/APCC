...
SF_CHUNK_ITERATOR *  wav_next_chunk_iterator(SF_PRIVATE *psf,
					     SF_CHUNK_ITERATOR * iterator) ;
int wav_get_chunk_size(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
		       SF_CHUNK_INFO * chunk_info) ;
int wav_get_chunk_data(SF_PRIVATE *psf, const SF_CHUNK_ITERATOR * iterator,
		       SF_CHUNK_INFO * chunk_info) ;
...
{	WAVLIKE_PRIVATE * wpriv ;
	...
	{	if ((error = wav_read_header (psf, &blockalign, &framesperblock)))
		...
		psf->next_chunk_iterator_signature = next_chunk_iterator_wav_next_chunk_iterator;
		psf->get_chunk_size = wav_get_chunk_size;
		psf->get_chunk_size_signature = get_chunk_size_wav_get_chunk_size;
		psf->get_chunk_data = wav_get_chunk_data;
		psf->get_chunk_data_signature = get_chunk_data_wav_get_chunk_data;
		} ;
	...
	{	if (psf->is_pipe)
		...
		*/
		...
		psf->write_header_signature = write_header_wav_write_header;
		psf->set_chunk = wav_set_chunk;
		psf->set_chunk_signature = set_chunk_wav_set_chunk;
		} ;

	psf->container_close = wav_close;
	psf->container_close_signature = container_close_wav_close;
	psf->command = wav_command;
	psf->command_signature = command_wav_command;
	...
	if (psf->file.mode == SFM_WRITE || (psf->file.mode == SFM_RDWR && psf->filelength == 0))
		...
		case write_header_aiff_write_header:
		    return aiff_write_header(psf, SF_FALSE);
		case write_header_au_write_header:
		    return au_write_header(psf, SF_FALSE);
		case write_header_avr_write_header:
		    return avr_write_header(psf, SF_FALSE);
		case write_header_caf_write_header:
		    return caf_write_header(psf, SF_FALSE);
		case write_header_htk_write_header:
		    return htk_write_header(psf, SF_FALSE);
		case write_header_ircam_write_header:
		    return ircam_write_header(psf, SF_FALSE);
		case write_header_mat4_write_header:
		    return mat4_write_header(psf, SF_FALSE);
		case write_header_mat5_write_header:
		    return mat5_write_header(psf, SF_FALSE);
		case write_header_mpc2k_write_header:
		    return mpc2k_write_header(psf, SF_FALSE);
		case write_header_nist_write_header:
		    return nist_write_header(psf, SF_FALSE);
		case write_header_paf_write_header:
		    return paf_write_header(psf, SF_FALSE);
		case write_header_pvf_write_header:
		    return pvf_write_header(psf, SF_FALSE);
		case write_header_rf64_write_header:
		    return rf64_write_header(psf, SF_FALSE);
		case write_header_sds_write_header:
		    return sds_write_header(psf, SF_FALSE);
		case write_header_svx_write_header:
		    return svx_write_header(psf, SF_FALSE);
		case write_header_voc_write_header:
		    return voc_write_header(psf, SF_FALSE);
		case write_header_w64_write_header:
		    return w64_write_header(psf, SF_FALSE);
		case write_header_wav_write_header:
		    return wav_write_header(psf, SF_FALSE);
		case write_header_wve_write_header:
		    return wve_write_header(psf, SF_FALSE);
		case write_header_xi_write_header:
		    return xi_write_header(psf, SF_FALSE);
		default:
					...
					{	int bytespersec, blockalign, flags, blocksize, samplesperblock, codecdelay ;
						...
						** byterate? */
						...
						case byterate_alac_byterate:
						    bytespersec = alac_byterate(psf);
						    break;
						case byterate_dwvw_byterate:
						    bytespersec = dwvw_byterate(psf);
						    break;
						case byterate_sds_byterate:
						    bytespersec = sds_byterate(psf);
						    break;
						default:
						    invalidFunctionWasCalled();
						    break;
...
int wav_command(SF_PRIVATE *psf, int command, void * UNUSED(data),
		int datasize)
{	WAVLIKE_PRIVATE	*wpriv ;
