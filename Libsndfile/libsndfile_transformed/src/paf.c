...
{	int		subformat, error, endian ;
	...
	{	if ((SF_CONTAINER (psf->sf.format)) != SF_FORMAT_PAF)
		...
		psf->write_header_signature = write_header_paf_write_header;
		} ;
...
int paf_write_header(SF_PRIVATE *psf, int UNUSED(calc_length))
{	int			paf_format ;
...
{	PAF24_PRIVATE	*ppaf24 ;
	...
	*/
	...
	{	paf24_read_block (psf, ppaf24) ;	/* Read first block. */
		...
		psf->read_short_signature = read_short_paf24_read_s;
		psf->read_int = paf24_read_i;
		psf->read_int_signature = read_int_paf24_read_i;
		psf->read_float = paf24_read_f;
		psf->read_float_signature = read_float_paf24_read_f;
		psf->read_double = paf24_read_d;
		psf->read_double_signature = read_double_paf24_read_d;
		} ;

	if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR)
	{	psf->write_short = paf24_write_s;
	psf->write_short_signature = write_short_paf24_write_s;
		psf->write_int = paf24_write_i;
		psf->write_int_signature = write_int_paf24_write_i;
		psf->write_float = paf24_write_f;
		psf->write_float_signature = write_float_paf24_write_f;
		psf->write_double = paf24_write_d;
		psf->write_double_signature = write_double_paf24_write_d;
		} ;

	psf->seek = paf24_seek;
	psf->seek_signature = seek_paf24_seek;
	psf->container_close = paf24_close;
	psf->container_close_signature = container_close_paf24_close;
