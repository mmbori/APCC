...
{	int				blocks ;
	...
	int decode_block_signature;
	int encode_block_signature;
} GSM610_PRIVATE ;
...
============================================================*/
	...
	{	case SF_FORMAT_WAV :
		...
		case SF_FORMAT_W64 :
			...
			pgsm610->encode_block_signature = encode_block_gsm610_wav_encode_block;
			pgsm610->decode_block = gsm610_wav_decode_block;
			pgsm610->decode_block_signature = decode_block_gsm610_wav_decode_block;
		...
		case SF_FORMAT_RAW :
			...
			pgsm610->encode_block_signature = encode_block_gsm610_encode_block;
			pgsm610->decode_block = gsm610_decode_block;
			pgsm610->decode_block_signature = decode_block_gsm610_decode_block;
	...
	{	if (psf->datalength % pgsm610->blocksize == 0)
		...
		switch (pgsm610->decode_block_signature) {
		case decode_block_gsm610_decode_block:
		    gsm610_decode_block(psf, pgsm610);
		    break;
		case decode_block_gsm610_wav_decode_block:
		    gsm610_wav_decode_block(psf, pgsm610);
		    break;
		default:
		...
		psf->read_short_signature = read_short_gsm610_read_s;
		psf->read_int = gsm610_read_i;
		psf->read_int_signature = read_int_gsm610_read_i;
		psf->read_float = gsm610_read_f;
		psf->read_float_signature = read_float_gsm610_read_f;
		psf->read_double = gsm610_read_d;
		psf->read_double_signature = read_double_gsm610_read_d;
		} ;
	...
	{	pgsm610->blockcount = 0 ;
		...
		psf->write_short_signature = write_short_gsm610_write_s;
		psf->write_int = gsm610_write_i;
		psf->write_int_signature = write_int_gsm610_write_i;
		psf->write_float = gsm610_write_f;
		psf->write_float_signature = write_float_gsm610_write_f;
		psf->write_double = gsm610_write_d;
		psf->write_double_signature = write_double_gsm610_write_d;
		} ;

	psf->codec_close = gsm610_close;
	psf->codec_close_signature = codec_close_gsm610_close;

	psf->seek = gsm610_seek;
	psf->seek_signature = seek_gsm610_seek;
...
sf_count_t gsm610_seek(SF_PRIVATE *psf, int UNUSED(mode), sf_count_t offset)
{	GSM610_PRIVATE *pgsm610 ;
...
{	int		count, total = 0, indx = 0 ;
	...
	{	count = pgsm610->samplesperblock - pgsm610->samplecount ;
		...
		if (pgsm610->samplecount >= pgsm610->samplesperblock)
			switch (pgsm610->encode_block_signature) {
			case encode_block_gsm610_encode_block:
			    gsm610_encode_block(psf, pgsm610);
			    break;
			case encode_block_gsm610_wav_encode_block:
			    gsm610_wav_encode_block(psf, pgsm610);
			    break;
			default:
			    invalidFunctionWasCalled();
			    break;
