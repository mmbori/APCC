...
// inlining_added
data_unset *array_data_string_copy(const data_unset *s);
data_unset *array_data_integer_copy(const data_unset *s);
data_unset *array_data_array_copy(const data_unset *s);
data_unset *data_config_copy(const data_unset *s);

static inline data_unset *
copy_dispatch(const data_unset *s) {
    data_unset *(*func)(const data_unset *) = s->fn->copy;
    
    if (func == array_data_string_copy) {
        return array_data_string_copy(s);
    }
    if (func == array_data_integer_copy) {
        return array_data_integer_copy(s);
    }
    if (func == array_data_array_copy) {
        return array_data_array_copy(s);
    }
    if (func == data_config_copy) {
        return data_config_copy(s);
    }
...
// inlining_added
void array_data_string_free(data_unset *du);
void array_data_integer_free(data_unset *du);
void array_data_array_free(data_unset *du);
void data_config_free(data_unset *du);
void data_auth_free(data_unset *du);

static inline void
free_dispatch(data_unset *du) {
    void (*func)(data_unset *) = du->fn->free;
    
    if (func == array_data_string_free) {
        array_data_string_free(du);
    }
    else if (func == array_data_integer_free) {
        array_data_integer_free(du);
    }
    else if (func == array_data_array_free) {
        array_data_array_free(du);
    }
    else if (func == data_config_free) {
        data_config_free(du);
    }
    else if (func == data_auth_free) {
        data_auth_free(du);
    }
}
...
static data_unset *configparser_get_variable(config_t *ctx, const buffer *key) {
  ...
  for (dc = ctx->current; dc; dc = dc->parent) {
    if (NULL != (du = array_get_element_klen(dc->value, BUF_PTR_LEN(key)))) {
      data_unset *du_copy = copy_dispatch(du);
      ...
      for (i = 0; i < src->used; i ++) {
        ...
        if (du) {
          if (buffer_is_unset(&du->key)
            ...
            array_insert_unique(dst, copy_dispatch(du));
          } else {
...
#line 449 "./configparser.y"
{
  ...
  if (yymsp[0].minor.yy35) free_dispatch(yymsp[0].minor.yy35);
  yymsp[0].minor.yy35 = NULL;
}
#line 1539 "./configparser.c"
...
#line 535 "./configparser.y"
{
  ...
  if (yymsp[-2].minor.yy35) free_dispatch(yymsp[-2].minor.yy35);
  yymsp[-2].minor.yy35 = NULL;
  if (yymsp[0].minor.yy35) free_dispatch(yymsp[0].minor.yy35);
  yymsp[0].minor.yy35 = NULL;
}
...
#line 705 "./configparser.y"
{
  ...
  if (ctx->ok) {
    ...
    if (NULL == (dc = configparser_get_data_config(ctx->all_configs, BUF_PTR_LEN(&yymsp[0].minor.yy20->key)))) {
      ...
      fprintf(stderr, "unreachable else condition\n");
      ctx->ok = 0;
      free_dispatch((data_unset *)yymsp[0].minor.yy20);
