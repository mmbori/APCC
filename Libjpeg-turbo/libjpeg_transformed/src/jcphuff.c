...
typedef struct {
  ...
  int AC_first_prepare_signature;
  int AC_refine_prepare_signature;
...
start_pass_phuff(j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  if (cinfo->Ah == 0) {
    if (is_DC_band) {
      ...
      entropy->pub.encode_mcu_signature = encode_mcu_encode_mcu_DC_first;
    }
    else {
      entropy->pub.encode_mcu = encode_mcu_AC_first;
      entropy->pub.encode_mcu_signature = encode_mcu_encode_mcu_AC_first;
    }
#ifdef WITH_SIMD
    if (jsimd_can_encode_mcu_AC_first_prepare()) {
      entropy->AC_first_prepare = jsimd_encode_mcu_AC_first_prepare;
      entropy->AC_first_prepare_signature = AC_first_prepare_jsimd_encode_mcu_AC_first_prepare;
    }
    else
#endif
      entropy->AC_first_prepare = encode_mcu_AC_first_prepare;
      entropy->AC_first_prepare_signature = AC_first_prepare_encode_mcu_AC_first_prepare;
  } else {
    if (is_DC_band) {
      entropy->pub.encode_mcu = encode_mcu_DC_refine;
      entropy->pub.encode_mcu_signature = encode_mcu_encode_mcu_DC_refine;
    }
    else {
      entropy->pub.encode_mcu = encode_mcu_AC_refine;
      entropy->pub.encode_mcu_signature = encode_mcu_encode_mcu_AC_refine;
#ifdef WITH_SIMD
      if (jsimd_can_encode_mcu_AC_refine_prepare()) {
        entropy->AC_refine_prepare = jsimd_encode_mcu_AC_refine_prepare;
        entropy->AC_refine_prepare_signature = AC_refine_prepare_jsimd_encode_mcu_AC_refine_prepare;
      }
      else
#endif
        entropy->AC_refine_prepare = encode_mcu_AC_refine_prepare;
        entropy->AC_refine_prepare_signature = AC_refine_prepare_encode_mcu_AC_refine_prepare;
  ...
  if (gather_statistics) {
    ...
    entropy->pub.finish_pass_signature = finish_pass_finish_pass_gather_phuff;
  }
  else {
    entropy->pub.finish_pass = finish_pass_phuff;
    entropy->pub.finish_pass_signature = finish_pass_finish_pass_phuff;
  }
...
encode_mcu_AC_first(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  /* Prepare data */
  // switch (entropy->AC_first_prepare_signature) {
  // case AC_first_prepare_encode_mcu_AC_first_prepare:
  //     encode_mcu_AC_first_prepare(MCU_data[0][0], jpeg_natural_order + cinfo->Ss,
  //                           Sl, Al, values, bits);
  //     break;
  // case AC_first_prepare_jsimd_encode_mcu_AC_first_prepare:
  //     jsimd_encode_mcu_AC_first_prepare(MCU_data[0][0], jpeg_natural_order + cinfo->Ss,
  //                           Sl, Al, values, bits);
  //     break;
  // default:
...
encode_mcu_AC_refine(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  /* Prepare data */
    // switch (entropy->AC_refine_prepare_signature) {
    // case AC_refine_prepare_encode_mcu_AC_refine_prepare:
    //     EOBPTR = absvalues + encode_mcu_AC_refine_prepare(MCU_data[0][0], jpeg_natural_order + cinfo->Ss,
    //                            Sl, Al, absvalues, bits);
    //     break;
    // case AC_refine_prepare_jsimd_encode_mcu_AC_refine_prepare:
    //     EOBPTR = absvalues + jsimd_encode_mcu_AC_refine_prepare(MCU_data[0][0], jpeg_natural_order + cinfo->Ss,
    //                            Sl, Al, absvalues, bits);
    //     break;
    // default:
    //     invalidFunctionWasCalled();
...
jinit_phuff_encoder(j_compress_ptr cinfo)
{
  ...
  entropy->pub.start_pass_signature = start_pass_start_pass_phuff;
