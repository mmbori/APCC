...
my_emit_message(j_common_ptr cinfo, int msg_level)
{
  if (msg_level < 0) {
    ...
    // switch (cinfo->err->error_exit_signature) {
    // case error_exit_fuzzer_error_exit:
    //     fuzzer_error_exit(cinfo);
    //     break;
    // case error_exit_my_error_exit:
    //     my_error_exit(cinfo);
    //     break;
    // default:
...
#endif
{
  ...
  if (strict) {
    ...
    jerr.emit_message_signature = emit_message_my_emit_message;
  }
  ...
  jerr.error_exit_signature = error_exit_fuzzer_error_exit;
  jerr.emit_message = fuzzer_emit_message;
  jerr.emit_message_signature = emit_message_fuzzer_emit_message;
