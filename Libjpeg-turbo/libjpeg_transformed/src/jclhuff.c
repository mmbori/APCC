...
start_pass_lhuff(j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  if (gather_statistics) {
    ...
    entropy->pub.encode_mcus_signature = encode_mcus_encode_mcus_gather;
    entropy->pub.finish_pass = finish_pass_gather;
    entropy->pub.finish_pass_signature = finish_pass_finish_pass_gather;
#else
    ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
  } else {
    entropy->pub.encode_mcus = encode_mcus_huff;
    entropy->pub.encode_mcus_signature = encode_mcus_encode_mcus_huff;
    entropy->pub.finish_pass = finish_pass_huff;
    entropy->pub.finish_pass_signature = finish_pass_finish_pass_huff;
  }
...
/* Empty the output buffer; return TRUE if successful, FALSE if must suspend */
{
  ...
  if (!(*dest->empty_output_buffer) (state->cinfo))
    return FALSE;
  // switch (dest->empty_output_buffer_signature) {
  //     case empty_output_buffer_empty_output_buffer :
  //       if (!empty_output_buffer (state->cinfo))
  //         return FALSE;
  //       break;
  //     case empty_output_buffer_empty_mem_output_buffer:
  //       if (!empty_mem_output_buffer (state->cinfo))
  //         return FALSE;
  //       break;
  //     default : invalidFunctionWasCalled();
  //   }
...
jinit_lhuff_encoder(j_compress_ptr cinfo)
{
  ...
  entropy->pub.start_pass_signature = start_pass_start_pass_lhuff;
