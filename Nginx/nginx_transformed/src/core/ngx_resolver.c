...
ngx_resolve_name(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->quick) {
        // ctx->handler(ctx);
                if (ctx->handler_signature == handler_ngx_channel_handler) {
            ngx_channel_handler(ctx);
        }
        else if (ctx->handler_signature == handler_NULL) {
            NULL;
        }
        else if (ctx->handler_signature == handler_ngx_cache_loader_process_handler) {
            ngx_cache_loader_process_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_cache_manager_process_handler) {
            ngx_cache_manager_process_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_clean_old_cycles) {
            ngx_clean_old_cycles(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_cleanup_environment) {
            ngx_cleanup_environment(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_cleanup_environment_variable) {
            ngx_cleanup_environment_variable(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_delete_udp_connection) {
            ngx_delete_udp_connection(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_epoll_notify_handler) {
            ngx_epoll_notify_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_event_accept) {
            ngx_event_accept(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_event_recvmsg) {
            ngx_event_recvmsg(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_close_connection) {
            ngx_http_close_connection(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_empty_gif_handler) {
            ngx_http_empty_gif_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_empty_handler) {
            ngx_http_empty_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_fastcgi_handler) {
            ngx_http_fastcgi_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_file_cache_cleanup) {
            ngx_http_file_cache_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_file_cache_lock_wait_handler) {
            ngx_http_file_cache_lock_wait_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_init_connection) {
            ngx_http_init_connection(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_keepalive_handler) {
            ngx_http_keepalive_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_limit_conn_cleanup) {
            ngx_http_limit_conn_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_lingering_close_handler) {
            ngx_http_lingering_close_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_log_flush_handler) {
            ngx_http_log_flush_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_memcached_handler) {
            ngx_http_memcached_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_process_request_headers) {
            ngx_http_process_request_headers(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_process_request_line) {
            ngx_http_process_request_line(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_proxy_handler) {
            ngx_http_proxy_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_request_handler) {
            ngx_http_request_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_scgi_handler) {
            ngx_http_scgi_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_set_lingering_close) {
            ngx_http_set_lingering_close(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_cleanup) {
            ngx_http_upstream_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_handler) {
            ngx_http_upstream_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_keepalive_close) {
            ngx_http_upstream_keepalive_close(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_keepalive_close_handler) {
            ngx_http_upstream_keepalive_close_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_keepalive_dummy_handler) {
            ngx_http_upstream_keepalive_dummy_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_resolve_handler) {
            ngx_http_upstream_resolve_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_zone_resolve_handler) {
            ngx_http_upstream_zone_resolve_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_upstream_zone_resolve_timer) {
            ngx_http_upstream_zone_resolve_timer(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_uwsgi_handler) {
            ngx_http_uwsgi_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_http_wait_request_handler) {
            ngx_http_wait_request_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_open_file_cache_cleanup) {
            ngx_open_file_cache_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_open_file_cache_remove) {
            ngx_open_file_cache_remove(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_open_file_cleanup) {
            ngx_open_file_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_pool_cleanup_file) {
            ngx_pool_cleanup_file(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_cleanup) {
            ngx_resolver_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_resend_handler) {
            ngx_resolver_resend_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_srv_names_handler) {
            ngx_resolver_srv_names_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_tcp_read) {
            ngx_resolver_tcp_read(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_tcp_write) {
            ngx_resolver_tcp_write(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_timeout_handler) {
            ngx_resolver_timeout_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_resolver_udp_read) {
            ngx_resolver_udp_read(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_shutdown_timer_handler) {
            ngx_shutdown_timer_handler(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_syslog_cleanup) {
            ngx_syslog_cleanup(ctx);
        }
        else if (ctx->handler_signature == handler_ngx_unload_module) {
            ngx_unload_module(ctx);
        }
        return NGX_OK;
    }
    ...
    u_char *query, u_short qlen)
{
    ...
    n = ngx_unix_send(rec->udp, query, qlen);
...
ngx_resolver_udp_read(ngx_event_t *rev)
{
    ...
    do {
        n = ngx_udp_unix_recv(c, buf, NGX_RESOLVER_UDP_SIZE);
...
ngx_resolver_tcp_write(ngx_event_t *wev)
{
    ...
    while (wev->ready && b->pos < b->last) {
        n = ngx_unix_send(c, b->pos, b->last - b->pos);
...
ngx_resolver_tcp_read(ngx_event_t *rev)
{
    ...
    while (rev->ready) {
        n = ngx_unix_recv(c, b->last, b->end - b->last);
...
ngx_udp_connect(ngx_resolver_connection_t *rec)
{
    ...
    if (ngx_epoll_add_event(rev, NGX_READ_EVENT, event) != NGX_OK) {
        goto failed;
    }
...
ngx_tcp_connect(ngx_resolver_connection_t *rec)
{
    ...
    if (rc == -1) {
        ...
        if (ngx_epoll_add_event(wev, NGX_WRITE_EVENT, event) != NGX_OK) {
            goto failed;
        }
