...
ngx_init_modules(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; cycle->modules[i]; i++) {
        if (cycle->modules[i]->init_module) {
            if (ngx_event_module_init(cycle) != NGX_OK) {
                return NGX_ERROR;
            }
        }
    }
    ...
    char **order)
{
    ...
    if (module->type == NGX_CORE_MODULE) {
        ...
        if (core_module->create_conf) {
            ...
            if (core_module->create_conf_signature == create_conf_NULL) {
                rv = NULL;
            }
            else if (core_module->create_conf_signature == create_conf_ngx_core_module_create_conf) {
                rv = ngx_core_module_create_conf(cf->cycle);
            }
            else if (core_module->create_conf_signature == create_conf_ngx_epoll_create_conf) {
                rv = ngx_epoll_create_conf(cf->cycle);
            }
            else if (core_module->create_conf_signature == create_conf_ngx_event_core_create_conf) {
                rv = ngx_event_core_create_conf(cf->cycle);
            }
            if (rv == NULL) {
                return NGX_ERROR;
            }
