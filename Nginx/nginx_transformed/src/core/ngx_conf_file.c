...
ngx_module_t  ngx_conf_module = {
    ...
    init_thread_NULL, 
    exit_thread_NULL,
    exit_process_ngx_conf_flush_files,
    exit_master_NULL
};
...
ngx_conf_flush_files(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (file[i].flush) {
            ngx_http_log_flush(&file[i], cycle->log);
        }
    }
}
...
ngx_conf_set_flag_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, fp);
    }
...
ngx_conf_set_str_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *field = value[1];
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, field);
    }
...
ngx_conf_set_str_array_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *s = value[1];
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, s);
    }
...
ngx_conf_set_keyval_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, kv);
    }
...
ngx_conf_set_num_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *np = ngx_atoi(value[1].data, value[1].len);
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, np);
    }
...
ngx_conf_set_size_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *sp = ngx_parse_size(&value[1]);
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, sp);
    }
...
ngx_conf_set_off_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *op = ngx_parse_offset(&value[1]);
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, op);
    }
...
ngx_conf_set_msec_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *msp = ngx_parse_time(&value[1], 0);
    ...
    if (cmd->post) {
        post = cmd->post;
        return ngx_conf_check_num_bounds(cf, post, msp);
    }
