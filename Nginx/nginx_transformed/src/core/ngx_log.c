...
#endif
{
    ...
    if (level != NGX_LOG_DEBUG && log->handler) {
        ...
        if (log->handler_signature == handler_NULL) {
                    p = NULL;
                }
                else if (log->handler_signature == handler_ngx_accept_log_error) {
                    p = ngx_accept_log_error(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_add_header) {
                    p = ngx_http_add_header(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_core_type) {
                    p = ngx_http_core_type(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_geo) {
                    p = ngx_http_geo(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_log_error) {
                    p = ngx_http_log_error(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_map) {
                    p = ngx_http_map(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_split_clients) {
                    p = ngx_http_split_clients(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_ssi_set_variable) {
                    p = ngx_http_ssi_set_variable(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_http_ssi_stub_output) {
                    p = ngx_http_ssi_stub_output(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_resolver_log_error) {
                    p = ngx_resolver_log_error(log, p, last - p);
                }
                else if (log->handler_signature == handler_ngx_syslog_log_error) {
                    p = ngx_syslog_log_error(log, p, last - p);
                }
    }
    ...
    while (log) {
        ...
        if (log->writer) {
            ngx_syslog_writer(log, level, errstr, p - errstr);
            goto next;
        }
