...
void ngx_http_upstream_upgraded_read_upstream(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
void ngx_http_upstream_upgraded_write_upstream(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
...
#endif
ngx_int_t ngx_http_upstream_output_filter(void *data,
    ngx_chain_t *chain);
void ngx_http_upstream_process_downstream(ngx_http_request_t *r);
void ngx_http_upstream_process_upstream(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
static void ngx_http_upstream_process_request(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
static void ngx_http_upstream_store(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
void ngx_http_upstream_dummy_handler(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
...
static ngx_http_module_t  ngx_http_upstream_module_ctx = {
    ...
    ,
    preconfiguration_ngx_http_upstream_add_variables,
    postconfiguration_NULL,
    create_main_conf_ngx_http_upstream_create_main_conf,
    init_main_conf_ngx_http_upstream_init_main_conf,
    create_srv_conf_NULL,
    merge_srv_conf_NULL,
    create_loc_conf_NULL,
    merge_loc_conf_NULL
};
...
ngx_http_upstream_init(ngx_http_request_t *r)
{
    ...
    if (ngx_event_flags & NGX_USE_CLEAR_EVENT) {
        ...
        if (!c->write->active) {
            if (ngx_epoll_add_event(c->write, NGX_WRITE_EVENT, NGX_CLEAR_EVENT)
                == NGX_ERROR)
            {
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    int tmp;
    if (u->create_request == ngx_http_fastcgi_create_request) {
        tmp = ngx_http_fastcgi_create_request(r);
    }
    else if (u->create_request == ngx_http_memcached_create_request) {
        tmp = ngx_http_memcached_create_request(r);
    }
    else if (u->create_request == ngx_http_proxy_create_request) {
        tmp = ngx_http_proxy_create_request(r);
    }
    else if (u->create_request == ngx_http_scgi_create_request) {
        tmp = ngx_http_scgi_create_request(r);
    }
    else if (u->create_request == ngx_http_uwsgi_create_request) {
        tmp = ngx_http_uwsgi_create_request(r);
    }
...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (c == NULL) {
        ...
        int tmp;
        if (u->create_key == ngx_http_fastcgi_create_key) {
            tmp = ngx_http_fastcgi_create_key(r);
        }
        else if (u->create_key == ngx_http_scgi_create_key) {
            tmp = ngx_http_scgi_create_key(r);
        }
        else if (u->create_key == ngx_http_proxy_create_key) {
            tmp = ngx_http_proxy_create_key(r);
        }
        else if (u->create_key == ngx_http_uwsgi_create_key) {
            tmp = ngx_http_uwsgi_create_key(r);
        }
...
ngx_http_upstream_cache_send(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (u->create_request == ngx_http_fastcgi_process_header) {
        rc = ngx_http_fastcgi_process_header(r);
    }
    else if (u->create_request == ngx_http_memcached_process_header) {
        rc = ngx_http_memcached_process_header(r);
    }
    else if (u->create_request == ngx_http_proxy_process_status_line) {
        rc = ngx_http_proxy_process_status_line(r);
    }
    else if (u->create_request == ngx_http_scgi_process_status_line) {
        rc = ngx_http_scgi_process_status_line(r);
    }
    else if (u->create_request == ngx_http_uwsgi_process_status_line) {
        rc = ngx_http_uwsgi_process_status_line(r);
    }
...
ngx_http_upstream_handler(ngx_event_t *ev)
{
    ...
    if (ev->write) {
        ...
        if (u->write_event_handler_signature == write_event_handler_ngx_http_upstream_dummy_handler) {
    ngx_http_upstream_dummy_handler(r, u);
}
else if (u->write_event_handler_signature == write_event_handler_ngx_http_upstream_send_request_handler) {
    ngx_http_upstream_send_request_handler(r, u);
}
else if (u->write_event_handler_signature == write_event_handler_ngx_http_upstream_upgraded_write_upstream) {
    ngx_http_upstream_upgraded_write_upstream(r, u);
}
    } else {
    ...
    ngx_event_t *ev)
{
    ...
    if (c->error) {
        if ((ngx_event_flags & NGX_USE_LEVEL_EVENT) && ev->active) {
            ...
            if (ngx_epoll_del_event(ev, event, 0) != NGX_OK) {
                ngx_http_upstream_finalize_request(r, u,
                                               NGX_HTTP_INTERNAL_SERVER_ERROR);
                return;
            }
...
ngx_http_upstream_reinit(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    int tmp;
    if (u->create_request == ngx_http_fastcgi_reinit_request) {
        tmp = ngx_http_fastcgi_reinit_request(r);
    }
    else if (u->create_request == ngx_http_memcached_reinit_request) {
        tmp = ngx_http_memcached_reinit_request(r);
    }
    else if (u->create_request == ngx_http_proxy_reinit_request) {
        tmp = ngx_http_proxy_reinit_request(r);
    }
    else if (u->create_request == ngx_http_scgi_reinit_request) {
        tmp = ngx_http_scgi_reinit_request(r);
    }
    else if (u->create_request == ngx_http_uwsgi_reinit_request) {
        tmp = ngx_http_uwsgi_reinit_request(r);
    }
...
ngx_http_upstream_test_next(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for (un = ngx_http_upstream_next_errors; un->status; un++) {
            ...
            && (u->conf->cache_use_stale & un->mask))
        {
            ...
            if (u->create_request == ngx_http_fastcgi_create_request) {
        rc = ngx_http_fastcgi_create_request(r);
    }
    else if (u->create_request == ngx_http_memcached_create_request) {
        rc = ngx_http_memcached_create_request(r);
    }
    else if (u->create_request == ngx_http_proxy_create_request) {
        rc = ngx_http_proxy_create_request(r);
    }
    else if (u->create_request == ngx_http_scgi_create_request) {
        rc = ngx_http_scgi_create_request(r);
    }
    else if (u->create_request == ngx_http_uwsgi_create_request) {
        rc = ngx_http_uwsgi_create_request(r);
    }
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (!u->buffering) {
        ...
        if (n) {
            ...
            int tmp;
            if (p->input_filter_signature == input_filter_ngx_event_pipe_copy_input_filter) {
                tmp = ngx_event_pipe_copy_input_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_fastcgi_input_filter) {
                tmp = ngx_http_fastcgi_input_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_fastcgi_non_buffered_filter) {
                tmp = ngx_http_fastcgi_non_buffered_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_memcached_filter) {
                tmp = ngx_http_memcached_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_proxy_chunked_filter) {
                tmp = ngx_http_proxy_chunked_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_proxy_copy_filter) {
                tmp = ngx_http_proxy_copy_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_proxy_non_buffered_chunked_filter) {
                tmp = ngx_http_proxy_non_buffered_chunked_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_proxy_non_buffered_copy_filter) {
                tmp = ngx_http_proxy_non_buffered_copy_filter(u->input_filter_ctx, n);
            }
            else if (p->input_filter_signature == input_filter_ngx_http_upstream_non_buffered_filter) {
                tmp = ngx_http_upstream_non_buffered_filter(u->input_filter_ctx, n);
            }
    ...
    for ( ;; ) {
        ...
        if (do_write) {
            ...
            if (size && dst->write->ready) {
                ...
                n = ngx_unix_send(dst, b->pos, size);
        ...
        if (size && src->read->ready) {
            ...
            n = ngx_unix_recv(src, b->last, size);
    ...
    ngx_uint_t do_write)
{
    ...
    for ( ;; ) {
        ...
        if (size && upstream->read->ready) {
            ...
            n = ngx_unix_recv(upstream, b->last, size);
            ...
            if (n > 0) {
                ...
                int tmp;
                if (u->input_filter_signature == input_filter_ngx_event_pipe_copy_input_filter) {
                tmp = ngx_event_pipe_copy_input_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_fastcgi_input_filter) {
                tmp = ngx_http_fastcgi_input_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_fastcgi_non_buffered_filter) {
                tmp = ngx_http_fastcgi_non_buffered_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_memcached_filter) {
                tmp = ngx_http_memcached_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_proxy_chunked_filter) {
                tmp = ngx_http_proxy_chunked_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_proxy_copy_filter) {
                tmp = ngx_http_proxy_copy_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_proxy_non_buffered_chunked_filter) {
                tmp = ngx_http_proxy_non_buffered_chunked_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_proxy_non_buffered_copy_filter) {
                tmp = ngx_http_proxy_non_buffered_copy_filter(u->input_filter_ctx, n);
            }
            else if (u->input_filter_signature == input_filter_ngx_http_upstream_non_buffered_filter) {
                tmp = ngx_http_upstream_non_buffered_filter(u->input_filter_ctx, n);
            }
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    *u->cleanup = NULL;
    ...
    if (u->finalize_request == ngx_http_fastcgi_finalize_request) {
            ngx_http_fastcgi_finalize_request(r, rc);
        }
        else if (u->finalize_request == ngx_http_memcached_finalize_request) {
            ngx_http_memcached_finalize_request(r, rc);
        }
        else if (u->finalize_request == ngx_http_proxy_finalize_request) {
            ngx_http_proxy_finalize_request(r, rc);
        }
        else if (u->finalize_request == ngx_http_scgi_finalize_request) {
            ngx_http_scgi_finalize_request(r, rc);
        }
        else if (u->finalize_request == ngx_http_uwsgi_finalize_request) {
            ngx_http_uwsgi_finalize_request(r, rc);
        }
    ...
    ngx_uint_t offset)
{
    ...
    *ho = *h;
    ...
    if (r->upstream->rewrite_redirect) {
        rc = ngx_http_proxy_rewrite_redirect(r, ho, 0);
    ...
    ngx_uint_t offset)
{
    ...
    *ho = *h;
    ...
    if (r->upstream->rewrite_redirect) {
        ...
        if (p) {
            rc = ngx_http_proxy_rewrite_redirect(r, ho, p + 4 - ho->value.data);
