...
typedef struct {
    ...
    ngx_uint_t                          rehash;
    uint32_t                            hash;
    ngx_int_t (*get_rr_peer)(ngx_peer_connection_t *pc,
    void *data);
...
ngx_int_t ngx_http_upstream_init_hash(ngx_conf_t *cf,
    ngx_http_upstream_srv_conf_t *us);
ngx_int_t ngx_http_upstream_init_hash_peer(ngx_http_request_t *r,
    ngx_http_upstream_srv_conf_t *us);
ngx_int_t ngx_http_upstream_get_hash_peer(ngx_peer_connection_t *pc,
    void *data);

ngx_int_t ngx_http_upstream_init_chash(ngx_conf_t *cf,
    ngx_http_upstream_srv_conf_t *us);
...
ngx_int_t ngx_http_upstream_init_chash_peer(ngx_http_request_t *r,
    ngx_http_upstream_srv_conf_t *us);
ngx_int_t ngx_http_upstream_get_chash_peer(ngx_peer_connection_t *pc,
    void *data);
...
static ngx_http_module_t  ngx_http_upstream_hash_module_ctx = {
    ...
    init_main_conf_NULL,
    create_srv_conf_ngx_http_upstream_hash_create_conf,
    merge_srv_conf_NULL,
...
ngx_http_upstream_get_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (hp->tries > 20 || hp->rrp.peers->number < 2 || hp->key.len == 0) {
        ...
        return ngx_http_upstream_get_round_robin_peer(pc, &hp->rrp);
    }
