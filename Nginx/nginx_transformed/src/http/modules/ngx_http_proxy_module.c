...
ngx_int_t ngx_http_proxy_copy_filter(ngx_event_pipe_t *p,
    ngx_buf_t *buf);
ngx_int_t ngx_http_proxy_chunked_filter(ngx_event_pipe_t *p,
    ngx_buf_t *buf);
ngx_int_t ngx_http_proxy_non_buffered_copy_filter(void *data,
    ssize_t bytes);
ngx_int_t ngx_http_proxy_non_buffered_chunked_filter(void *data,
    ssize_t bytes);
...
void ngx_http_proxy_finalize_request(ngx_http_request_t *r,
    ngx_int_t rc);
...
char *ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf,
...
static ngx_http_module_t  ngx_http_proxy_module_ctx = {
    ...
    ,
    preconfiguration_ngx_http_proxy_add_variables,
    postconfiguration_NULL,
    create_main_conf_ngx_http_proxy_create_main_conf,
    init_main_conf_NULL,
    create_srv_conf_NULL,
    merge_srv_conf_NULL,
    create_loc_conf_ngx_http_proxy_create_loc_conf,
    merge_loc_conf_ngx_http_proxy_merge_loc_conf
};
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    if (plcf->body_lengths) {
        ...
        while (*(uintptr_t *) le.ip) {
            ...
            body_len +=  ngx_http_script_copy_len_code(&le);
            }
            else if (lcode == ngx_http_script_copy_var_len_code) {
                body_len +=  ngx_http_script_copy_var_len_code(&le);
            }
            else if (lcode == ngx_http_script_full_name_len_code) {
                body_len +=  ngx_http_script_full_name_len_code(&le);
            }
        }
    ...
    *b->last++ = ' ';
    ...
    while (*(uintptr_t *) le.ip) {
        ...
        // (void) lcode(&le);
        if (lcode == ngx_http_script_copy_len_code) {
            (void) ngx_http_script_copy_len_code(&le);
            }
            else if (lcode == ngx_http_script_copy_var_len_code) {
                (void) ngx_http_script_copy_var_len_code(&le);
            }
            else if (lcode == ngx_http_script_full_name_len_code) {
                (void)  ngx_http_script_full_name_len_code(&le);
            }
