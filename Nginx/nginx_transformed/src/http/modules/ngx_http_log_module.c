...
struct ngx_http_log_op_s {
    size_t                      len;
    size_t (*getlen) (ngx_http_request_t *r,
    uintptr_t data);
    u_char *(*run) (ngx_http_request_t *r, u_char *buf,
    ngx_http_log_op_t *op);
    uintptr_t                   data;

    int getlen_signature;
    int run_signature;
};
...
char *ngx_http_log_merge_loc_conf(ngx_conf_t *cf, void *parent,
    void *child);
...
static ngx_http_module_t  ngx_http_log_module_ctx = {
    ...
    postconfiguration_ngx_http_log_init,
    create_main_conf_ngx_http_log_create_main_conf,
    init_main_conf_NULL,
    create_srv_conf_NULL,
    merge_srv_conf_NULL,
    create_loc_conf_ngx_http_log_create_loc_conf,
    merge_loc_conf_ngx_http_log_merge_loc_conf
};
    ...
    for (l = 0; l < lcf->logs->nelts; l++) {
        ...
        for (i = 0; i < log[l].format->ops->nelts; i++) {
            if (op[i].len == 0) {
                ...
                if (op[i].getlen_signature == getlen_NULL) {
    len += NULL;
}
else if (op[i].getlen_signature == getlen_ngx_http_log_json_variable_getlen) {
    len += ngx_http_log_json_variable_getlen(r, op[i].data);
}
else if (op[i].getlen_signature == getlen_ngx_http_log_unescaped_variable_getlen) {
    len += ngx_http_log_unescaped_variable_getlen(r, op[i].data);
}
else if (op[i].getlen_signature == getlen_ngx_http_log_variable_getlen) {
    len += ngx_http_log_variable_getlen(r, op[i].data);
}
        ...
        if (buffer) {
            ...
            if (len <= (size_t) (buffer->last - buffer->pos)) {
                ...
                for (i = 0; i < log[l].format->ops->nelts; i++) {
                    p = ngx_http_log_unescaped_variable(r, p, &op[i]);
                }
