...
ngx_int_t ngx_http_ssi_stub_output(ngx_http_request_t *r, void *data,
    ngx_int_t rc);
ngx_int_t ngx_http_ssi_set_variable(ngx_http_request_t *r, void *data,
    ngx_int_t rc);
...
char *ngx_http_ssi_merge_loc_conf(ngx_conf_t *cf,
...
static ngx_http_module_t  ngx_http_ssi_filter_module_ctx = {
    ...
    ,
    preconfiguration_ngx_http_ssi_preconfiguration,
    postconfiguration_ngx_http_ssi_filter_init,
    create_main_conf_ngx_http_ssi_create_main_conf,
    init_main_conf_ngx_http_ssi_init_main_conf,
    create_srv_conf_NULL,
    merge_srv_conf_NULL,
    create_loc_conf_ngx_http_ssi_create_loc_conf,
    merge_loc_conf_ngx_http_ssi_merge_loc_conf
};
...
ngx_http_ssi_header_filter(ngx_http_request_t *r)
{
        ...
        || ngx_http_test_content_type(r, &slcf->types) == NULL)
    {
        ...
        if (ngx_http_next_header_filter == ngx_http_header_filter) {
            return ngx_http_header_filter(r);
        }
        else if (ngx_http_next_header_filter == ngx_http_chunked_header_filter) {
            return ngx_http_chunked_header_filter(r);
        }
        else if (ngx_http_next_header_filter == ngx_http_headers_filter) {
            return ngx_http_headers_filter(r);
        }
        else if (ngx_http_next_header_filter == ngx_http_not_modified_header_filter) {
            return ngx_http_not_modified_header_filter(r);
        }
        else if (ngx_http_next_header_filter == ngx_http_range_header_filter) {
            return ngx_http_range_header_filter(r);
        }
        else if (ngx_http_next_header_filter == ngx_http_ssi_header_filter) {
            return ngx_http_ssi_header_filter(r);
        }
        else if (ngx_http_next_header_filter == ngx_http_userid_filter) {
...
ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
            ...
            && ctx->busy == NULL))
    {
        ...
        if (ngx_http_next_body_filter == ngx_http_chunked_body_filter) {
            return ngx_http_chunked_body_filter(r, in);
        }
        else if (ngx_http_next_body_filter == ngx_http_trailers_filter) {
            return ngx_http_trailers_filter(r, in);
        }
        else if (ngx_http_next_body_filter == ngx_http_ssi_body_filter) {
            return ngx_http_ssi_body_filter(r, in);
        }
        else if (ngx_http_next_body_filter == ngx_http_copy_filter) {
            return ngx_http_copy_filter(r, in);
        }
        else if (ngx_http_next_body_filter == ngx_http_postpone_filter) {
            return ngx_http_postpone_filter(r, in);
        }
        else if (ngx_http_next_body_filter == ngx_http_write_filter) {
            return ngx_http_write_filter(r, in);
        }
        else if (ngx_http_next_body_filter == ngx_http_range_body_filter) {
            return ngx_http_range_body_filter(r, in);
        }
    }
    ...
    if (ctx->wait) {
        ...
        if (ctx->wait->done) {
            ...
            if (ngx_http_next_body_filter == ngx_http_chunked_body_filter) {
            return ngx_http_chunked_body_filter(r, NULL);
        }
        else if (ngx_http_next_body_filter == ngx_http_trailers_filter) {
            return ngx_http_trailers_filter(r, NULL);
        }
        else if (ngx_http_next_body_filter == ngx_http_ssi_body_filter) {
            return ngx_http_ssi_body_filter(r, NULL);
        }
        else if (ngx_http_next_body_filter == ngx_http_copy_filter) {
            return ngx_http_copy_filter(r, NULL);
        }
        else if (ngx_http_next_body_filter == ngx_http_postpone_filter) {
            return ngx_http_postpone_filter(r, NULL);
        }
        else if (ngx_http_next_body_filter == ngx_http_write_filter) {
            return ngx_http_write_filter(r, NULL);
        }
        else if (ngx_http_next_body_filter == ngx_http_range_body_filter) {
            return ngx_http_range_body_filter(r, NULL);
        }
        }
    }
    ...
    while (ctx->in || ctx->buf) {
        ...
        while (ctx->pos < ctx->buf->last) {
            ...
            if (rc == NGX_OK) {
                ...
                if (cmd->handler_signature == handler_NULL) {
                    rc = NULL;
                }
                else if (cmd->handler_signature == handler_ngx_accept_log_error) {
                    rc = ngx_accept_log_error(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_add_header) {
                    rc = ngx_http_add_header(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_core_type) {
                    rc = ngx_http_core_type(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_geo) {
                    rc = ngx_http_geo(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_log_error) {
                    rc = ngx_http_log_error(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_map) {
                    rc = ngx_http_map(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_split_clients) {
                    rc = ngx_http_split_clients(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_ssi_set_variable) {
                    rc = ngx_http_ssi_set_variable(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_http_ssi_stub_output) {
                    rc = ngx_http_ssi_stub_output(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_resolver_log_error) {
                    rc = ngx_resolver_log_error(r, ctx, params);
                }
                else if (cmd->handler_signature == handler_ngx_syslog_log_error) {
                    rc = ngx_syslog_log_error(r, ctx, params);
                }
...
ngx_http_ssi_output(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx)
{
    ...
    if (ngx_http_next_body_filter == ngx_http_chunked_body_filter) {
            rc = ngx_http_chunked_body_filter(r, ctx->out);
        }
        else if (ngx_http_next_body_filter == ngx_http_trailers_filter) {
            rc = ngx_http_trailers_filter(r, ctx->out);
        }
        else if (ngx_http_next_body_filter == ngx_http_ssi_body_filter) {
            rc = ngx_http_ssi_body_filter(r, ctx->out);
        }
        else if (ngx_http_next_body_filter == ngx_http_copy_filter) {
            rc = ngx_http_copy_filter(r, ctx->out);
        }
        else if (ngx_http_next_body_filter == ngx_http_postpone_filter) {
            rc = ngx_http_postpone_filter(r, ctx->out);
        }
        else if (ngx_http_next_body_filter == ngx_http_write_filter) {
            rc = ngx_http_write_filter(r, ctx->out);
        }
        else if (ngx_http_next_body_filter == ngx_http_range_body_filter) {
            rc = ngx_http_range_body_filter(r, ctx->out);
        }
