...
void ngx_http_uwsgi_finalize_request(ngx_http_request_t *r,
    ngx_int_t rc);

void *ngx_http_uwsgi_create_main_conf(ngx_conf_t *cf);
void *ngx_http_uwsgi_create_loc_conf(ngx_conf_t *cf);
char *ngx_http_uwsgi_merge_loc_conf(ngx_conf_t *cf, void *parent,
    void *child);
...
static ngx_http_module_t ngx_http_uwsgi_module_ctx = {
    ...
    create_main_conf_ngx_http_uwsgi_create_main_conf,
    init_main_conf_NULL,
    create_srv_conf_NULL,
    merge_srv_conf_NULL,
    create_loc_conf_ngx_http_uwsgi_create_loc_conf,
    merge_loc_conf_ngx_http_uwsgi_merge_loc_conf
};
...
ngx_http_uwsgi_handler(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_read_client_request_body(r, ngx_http_upstream_init, post_handler_ngx_http_upstream_init);
...
ngx_http_uwsgi_create_request(ngx_http_request_t *r)
{
    ...
    if (params->lengths) {
        ...
        while (*(uintptr_t *) le.ip) {
            ...
            // key_len = lcode(&le);
            if (lcode == ngx_http_script_copy_len_code) {
            key_len =  ngx_http_script_copy_len_code(&le);
            }
            else if (lcode == ngx_http_script_copy_var_len_code) {
                key_len =  ngx_http_script_copy_var_len_code(&le);
            }
            else if (lcode == ngx_http_script_full_name_len_code) {
                key_len =  ngx_http_script_full_name_len_code(&le);
            }

            lcode = *(ngx_http_script_len_code_pt *) le.ip;
            // skip_empty = lcode(&le);
            if (lcode == ngx_http_script_copy_len_code) {
            skip_empty =  ngx_http_script_copy_len_code(&le);
            }
            else if (lcode == ngx_http_script_copy_var_len_code) {
                skip_empty =  ngx_http_script_copy_var_len_code(&le);
            }
            else if (lcode == ngx_http_script_full_name_len_code) {
                skip_empty =  ngx_http_script_full_name_len_code(&le);
            }

            if (lcode == ngx_http_script_copy_len_code) {
                for (val_len = 0; *(uintptr_t *) le.ip; val_len += ngx_http_script_copy_len_code(&le)) {
                    lcode = *(ngx_http_script_len_code_pt *) le.ip;
                }
            }
            else if (lcode == ngx_http_script_copy_var_len_code) {
                for (val_len = 0; *(uintptr_t *) le.ip; val_len += ngx_http_script_copy_var_len_code(&le)) {
                    lcode = *(ngx_http_script_len_code_pt *) le.ip;
                }
            }
            else if (lcode == ngx_http_script_full_name_len_code) {
                for (val_len = 0; *(uintptr_t *) le.ip; val_len += ngx_http_script_full_name_len_code(&le)) {
                    lcode = *(ngx_http_script_len_code_pt *) le.ip;
                }
            }
    ...
    *b->last++ = (u_char) uwcf->modifier2;
    ...
    if (params->lengths) {
        ...
        while (*(uintptr_t *) le.ip) {
            ...
            if (lcode == ngx_http_script_copy_len_code) {
            key_len =  (u_char) ngx_http_script_copy_len_code(&le);
            }
            else if (lcode == ngx_http_script_copy_var_len_code) {
                key_len =  (u_char) ngx_http_script_copy_var_len_code(&le);
            }
            else if (lcode == ngx_http_script_full_name_len_code) {
                key_len =  (u_char) ngx_http_script_full_name_len_code(&le);
            }
            ...
            if (skip_empty && val_len == 0) {
                ...
                while (*(uintptr_t *) e.ip) {
                    ...
                    if (code == ngx_http_script_copy_code) {
            ngx_http_script_copy_code(&e);
        }
        else if (code == ngx_http_script_copy_var_code) {
            ngx_http_script_copy_var_code(&e);
        }
        else if (code == ngx_http_script_full_name_code) {
            ngx_http_script_full_name_code(&e);
        }
                }
