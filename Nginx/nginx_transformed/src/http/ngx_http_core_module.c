...
char *ngx_http_core_merge_srv_conf(ngx_conf_t *cf,
    void *parent, void *child);
void *ngx_http_core_create_loc_conf(ngx_conf_t *cf);
char *ngx_http_core_merge_loc_conf(ngx_conf_t *cf,
    void *parent, void *child);
...
static char *ngx_http_core_types(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
char *ngx_http_core_type(ngx_conf_t *cf, ngx_command_t *dummy,
    void *conf);
...
static ngx_http_module_t  ngx_http_core_module_ctx = {
    ...
    ,
    preconfiguration_ngx_http_core_preconfiguration,
    postconfiguration_ngx_http_core_postconfiguration,
    create_main_conf_ngx_http_core_create_main_conf,
    init_main_conf_ngx_http_core_init_main_conf,
    create_srv_conf_ngx_http_core_create_srv_conf,
    merge_srv_conf_ngx_http_core_merge_srv_conf,
    create_loc_conf_ngx_http_core_create_loc_conf,
    merge_loc_conf_ngx_http_core_merge_loc_conf
};
...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    while (ph[r->phase_handler].checker) {
        ...
        if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_find_config_phase) {
            rc =ngx_http_core_find_config_phase(r, &ph[r->phase_handler]);
        }
        else if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_post_access_phase) {
            rc =ngx_http_core_post_access_phase(r, &ph[r->phase_handler]);
        }
        else if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_post_rewrite_phase) {
            rc =ngx_http_core_post_rewrite_phase(r, &ph[r->phase_handler]);
        }
        else if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_rewrite_phase) {
            rc =ngx_http_core_rewrite_phase(r, &ph[r->phase_handler]);
        }
        else if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_content_phase) {
            rc =ngx_http_core_content_phase(r, &ph[r->phase_handler]);
        }
        else if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_generic_phase) {
            rc =ngx_http_core_generic_phase(r, &ph[r->phase_handler]);
        }
        else if (ph[r->phase_handler].checker_signature == checker_ngx_http_core_access_phase) {
            rc =ngx_http_core_access_phase(r, &ph[r->phase_handler]);
        }
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    rc = ph->handler(r);
    // if (ph->handler_signature == handler_NULL) {
    //         rc = NULL;
    //     }
    //     else if (ph->handler_signature == handler_ngx_http_empty_gif_handler) {
    //         rc = ngx_http_empty_gif_handler(r);
    //     }
    //     else if (ph->handler_signature == handler_ngx_http_fastcgi_handler) {
    //         rc = ngx_http_fastcgi_handler(r);
    //     }
    //     else if (ph->handler_signature == handler_ngx_http_memcached_handler) {
    //         rc = ngx_http_memcached_handler(r);
    //     }
    //     else if (ph->handler_signature == handler_ngx_http_proxy_handler) {
    //         rc = ngx_http_proxy_handler(r);
    //     }
    //     else if (ph->handler_signature == handler_ngx_http_scgi_handler) {
    //         rc = ngx_http_scgi_handler(r);
    //     }
    //     else if (ph->handler_signature == handler_ngx_http_uwsgi_handler) {
    //         rc = ngx_http_uwsgi_handler(r);
    //     }
...
ngx_http_send_header(ngx_http_request_t *r)
{
    ...
    if (ngx_http_top_header_filter == ngx_http_header_filter) {
            return ngx_http_header_filter(r);
        }
        else if (ngx_http_top_header_filter == ngx_http_chunked_header_filter) {
            return ngx_http_chunked_header_filter(r);
        }
        else if (ngx_http_top_header_filter == ngx_http_headers_filter) {
            return ngx_http_headers_filter(r);
        }
        else if (ngx_http_top_header_filter == ngx_http_not_modified_header_filter) {
            return ngx_http_not_modified_header_filter(r);
        }
        else if (ngx_http_top_header_filter == ngx_http_range_header_filter) {
            return ngx_http_range_header_filter(r);
        }
        else if (ngx_http_top_header_filter == ngx_http_ssi_header_filter) {
            return ngx_http_ssi_header_filter(r);
        }
        else if (ngx_http_top_header_filter == ngx_http_userid_filter) {
            return ngx_http_userid_filter(r);
        }
...
ngx_http_send_early_hints(ngx_http_request_t *r)
{
    ...
    return ngx_http_early_hints_filter(r);
}
...
ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ngx_http_top_body_filter == ngx_http_chunked_body_filter) {
            rc = ngx_http_chunked_body_filter(r, in);
        }
        else if (ngx_http_top_body_filter == ngx_http_trailers_filter) {
            rc = ngx_http_trailers_filter(r, in);
        }
        else if (ngx_http_top_body_filter == ngx_http_ssi_body_filter) {
            rc = ngx_http_ssi_body_filter(r, in);
        }
        else if (ngx_http_top_body_filter == ngx_http_copy_filter) {
            rc = ngx_http_copy_filter(r, in);
        }
        else if (ngx_http_top_body_filter == ngx_http_postpone_filter) {
            rc = ngx_http_postpone_filter(r, in);
        }
        else if (ngx_http_top_body_filter == ngx_http_write_filter) {
            rc = ngx_http_write_filter(r, in);
        }
        else if (ngx_http_top_body_filter == ngx_http_range_body_filter) {
            rc = ngx_http_range_body_filter(r, in);
        }
...
//             if (module->create_loc_conf_signature == create_loc_conf_ngx_http_access_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_access_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_auth_basic_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_auth_basic_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_autoindex_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_autoindex_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_browser_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_browser_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_copy_filter_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_copy_filter_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_core_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_core_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_fastcgi_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_fastcgi_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_headers_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_headers_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_index_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_index_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_limit_conn_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_limit_conn_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_limit_req_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_limit_req_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_log_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_log_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_memcached_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_memcached_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_mirror_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_mirror_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_proxy_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_proxy_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_referer_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_referer_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_scgi_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_scgi_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_ssi_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_ssi_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_try_files_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_try_files_create_loc_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_userid_create_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_userid_create_conf(cf);
// }
// else if (module->create_loc_conf_signature == create_loc_conf_ngx_http_uwsgi_create_loc_conf) {
//     ctx->loc_conf[cf->cycle->modules[i]->ctx_index] =ngx_http_uwsgi_create_loc_conf(cf);
// }
