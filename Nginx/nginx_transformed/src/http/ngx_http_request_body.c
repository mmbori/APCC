...
ngx_http_read_client_request_body(ngx_http_request_t *r,
    void (*post_handler)(ngx_http_request_t *r), int post_handler_signature)
{
    size_t                     preread;
    ssize_t                    size;
    ngx_int_t                  rc;
    ...
    if (r != r->main || r->request_body || r->discard_body) {
        ...
        // post_handler(r);
        if (post_handler_signature == post_handler_ngx_http_mirror_body_handler) {
    ngx_http_mirror_body_handler(r);
}
else if (post_handler_signature == post_handler_ngx_http_upstream_init) {
    ...
    rb->post_handler_signature = post_handler_signature;
...
ngx_http_do_read_client_request_body(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        for ( ;; ) {
            ...
            n = ngx_unix_recv(c, rb->buf->last, size);
    ...
    if (!r->request_body_no_buffering) {
        r->read_event_handler = ngx_http_block_reading;
        // rb->post_handler(r);
        if (rb->post_handler_signature == post_handler_ngx_http_mirror_body_handler) {
    ngx_http_mirror_body_handler(r);
}
else if (rb->post_handler_signature == post_handler_ngx_http_upstream_init) {
    ngx_http_upstream_init(r);
}
    }
...
ngx_http_read_discarded_request_body(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        ...
        n = ngx_unix_recv(r->connection, buffer, size);
