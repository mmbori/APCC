...
PIN_OUTPUT="${PIN_OUTPUT:-nginx_pin_log.txt}"
...
echo -n "Nginx 서버 시작 (with Pin)... "
"$PIN_ROOT/pin" -t "$PINTOOL" -o "$PIN_OUTPUT" -- \
    "$NGINX_BIN" -c "$CONFIG_FILE" 2>&1 | grep -v -E "\[CFI Checking Counter\]|PID:|Total CFI|Unique CFI|Output file:|========" &
NGINX_PID=$!
sleep 1
...
http_get() {
    local path="$1"
    curl -s --connect-timeout 1 --max-time 2 "http://127.0.0.1:$HTTP_PORT$path" -o /dev/null -w '%{http_code}' 2>/dev/null || echo "TIMEOUT"
}

http_head() {
    local path="$1"
    curl -s --connect-timeout 1 --max-time 2 -I "http://127.0.0.1:$HTTP_PORT$path" 2>/dev/null || echo "TIMEOUT"
}

http_get_content() {
    local path="$1"
    curl -s --connect-timeout 1 --max-time 2 "http://127.0.0.1:$HTTP_PORT$path" 2>/dev/null || echo "TIMEOUT"
}

http_method() {
    local method="$1"
    local path="$2"
    curl -s --connect-timeout 1 --max-time 2 -X "$method" "http://127.0.0.1:$HTTP_PORT$path" -o /dev/null -w '%{http_code}' 2>/dev/null || echo "TIMEOUT"
}
...
run_test() {
  ...
  if echo "$result" | grep -q "$expected"; then
    ...
    echo -e "${YELLOW}[$section]${NC} $description ${RED}✘${NC} (got: $result, expected: $expected)"
    FAIL=$((FAIL+1))
  fi
}

echo "================================================"
echo "1. 기본 GET 요청 테스트 (20개)"
echo "================================================"
run_test "1.1" "GET /" "http_get /" "200"
...
MAIN_PIN_FILE="${PIN_OUTPUT}.${NGINX_PID}"
